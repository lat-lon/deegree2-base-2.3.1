...
<xsl:stylesheet
  ...
  <!-- brings a posList text into coordinates compatible format -->
  <xsl:template name="tupleSeparator">
    <xsl:param name="tuples" />
    <xsl:param name="dimension" />
    <xsl:if test="string-length($tuples) != 0">
      <xsl:value-of select="substring-before($tuples, ' ')" />
      <xsl:text>,</xsl:text>
      <xsl:variable name="theval2" select="substring-before(substring-after($tuples, ' '), ' ')" />
      <xsl:choose>
        <xsl:when test="string-length($theval2) = 0">
          <xsl:value-of select="substring-after($tuples, ' ')" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$theval2" />
        </xsl:otherwise>
      </xsl:choose>
      <xsl:if test="$dimension = 3">
        <xsl:text>,</xsl:text>
        <xsl:variable name="theval" select="substring-before(substring-after(substring-after($tuples, ' '), ' '), ' ')" />
        <xsl:choose>
          <xsl:when test="string-length($theval) = 0">
            <xsl:value-of select="substring-after(substring-after($tuples, ' '), ' ')" />
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:variable name="rest">
        <xsl:choose>
          <xsl:when test="$dimension = 3">
            <xsl:value-of select="substring-after(substring-after(substring-after($tuples, ' '), ' '), ' ')" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="substring-after(substring-after($tuples, ' '), ' ')" />
          </xsl:otherwise>
      ...
      <xsl:call-template name="tupleSeparator">
        <xsl:with-param name="dimension" select="$dimension" />
        <xsl:with-param name="tuples" select="$rest" />
      </xsl:call-template>
    </xsl:if>
  </xsl:template>
      ...
      <xsl:call-template name="tupleSeparator">
        <xsl:with-param name="dimension" >
          <xsl:choose>
            ...
            <xsl:when test="boolean(@count)">
              <xsl:value-of select="@count" />
            </xsl:when>
            <xsl:otherwise>2</xsl:otherwise>
          </xsl:choose>
        </xsl:with-param>
        <xsl:with-param name="tuples" select="." />
      </xsl:call-template>
    </gml:coordinates>
  </xsl:template>
