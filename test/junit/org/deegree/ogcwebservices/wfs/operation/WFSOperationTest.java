//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/test/junit/org/deegree/ogcwebservices/wfs/operation/WFSOperationTest.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class WFSOperationTest extends TestCase {
    ...
    // File filterbase = new File( new URL( Configuration.getWFSBaseDir(), REQUEST_DIR
    // + "Transaction/xml" ).getFile() );
    // FileFilter xmlFilter = new ExtensionFilter( "xml" );
    // File[] fileList = filterbase.listFiles( xmlFilter );
    // for (int i = 0; i < fileList.length; i++) {
    // LOG.logInfo( "Parsing XML encoded example Transaction request: "
    // + fileList[i] );
    // Document doc = XMLTools.parse( new FileReader( fileList[i] ) );
    // Transaction request = Transaction.create( ""
    // + i, doc );
    // assertNotNull( request );
    // assertEquals( "WFS", request.getService() );
    // }
    // }
