 ...
 * 
 * @version $Revision: 1.7 $, $Date: 2007-06-21 13:54:33 $
 * 
 */
...
public class InsertTransactionHandler {
    ...
    public List<Element> handleInsertTransaction( Manager transactionManager, int[] resultValues )
        ...
        Expression iduriExpr = new PropertyName( new QualifiedName( "app", "liduri", appURI ) );
         ...
         PropertyIsCOMPOperation validOperator = new PropertyIsCOMPOperation( OperationDefines.PROPERTYISEQUALTO, statusExpr, new Literal( "valid" ) );
        
         PropertyIsCOMPOperation emptyOperator = new PropertyIsCOMPOperation( OperationDefines.PROPERTYISEQUALTO, statusExpr, new Literal( "" ) );
        ...
        WFService localWFS = transactionManager.getWfsService();
...
//            Expression idLiteral = new Literal( oldID );
            ...
            if ( featureCollectionOnId == null || "".equals( featureCollectionOnId ) ) {
            ...
            try {
                ...
                LOG.logDebug( " InsertHandler: the number of features in the GetFeatureWithLock was: "
                              + featureCount );
            ...
            if ( featureCount > 1 ) {
                ...
                int totalUpdated = changeStatusOfObject( lockId, registryObject, idAndStatusFilter, record.getNodeName(),
                                                         localWFS );
                ...
                if( iduriProperty == null ){
                    LOG.logError( "The id of this element: " + record.getNodeName()
                    ...
                    if(  predecessorID == null || "".equals( predecessorID.trim() ) ){
                        LOG.logError( "The registry helds an id of this element: " + record.getNodeName()
                    ...
                    LOG.logDebug( " wcsFilter: total updated wfs:records (should be >= 1) = "
                                  + totalUpdated );
                    if ( totalUpdated == 1 ) {
                ...
                for ( int i = ( recordCount + 1 ); i < records.size(); ++i ) {
                    ...
                    if ( CommonNamespaces.OASIS_EBRIMNS.toASCIIString().equals( tmpRec.getNamespaceURI() )
                        ...
                        if ( oldID.equals( sourceObject ) ) {
                            LOG.logDebug( " Updating 'rim:Association/@sourceObject' Attribute to new id: "
                                          + newID + " after an update of registryObject: " + record.getLocalName() );
                            tmpRec.setAttribute( "sourceObject", newID );
                        }
                        if ( oldID.equals( targetObject ) ) {
                            LOG.logDebug( " Updating 'rim:Association/@targetObject' Attribute to new id: "
                                          + newID + " after an update of registryObject: " + record.getLocalName() );
    ...
    private FeatureResult sendWFSGetFeature( WFService localWFS, QualifiedName registryObject, ComplexFilter filter )
        ...
        if ( LOG.getLevel() == ILogger.LOG_DEBUG ) {
            try {
                ...
                LOG.logDebug( "The getFeature with lock request: "
    ...
    private Element generateBriefRecord( Element record ) {
        ...
        if ( attribs != null ) {
            for ( Node attribute : attribs ) {
                ...
                LOG.logDebug( "From: " + a.getNodeName()
                              + " found attribute (localname): " + localName );
                if ( !( "id".equals( localName ) || "lid".equals( localName ) || "objectType".equals( localName ) || "status".equals( localName ) ) ) {
                    ...
                    LOG.logDebug( " From: " + a.getNodeName()
                                  + " removing attribute (localname): " + localName );
    ...
    private void insertFeatures( WFService localWFS, List<Feature> featuresToInsert, String originalInsertObjectName )
        ...
        if ( LOG.getLevel() == ILogger.LOG_DEBUG ) {
            try {
                ...
                LOG.logDebug( " The auditable event created for the insertion of '"
                              + originalInsertObjectName + "' is:\n" + doc.getAsPrettyString() );
            } catch ( IOException e ) {
