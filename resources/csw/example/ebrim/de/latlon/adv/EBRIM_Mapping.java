 ...
 * 
 * @version 1.0. $Revision: 1.8 $, $Date: 2007-06-21 13:53:57 $
 * 
 * @since 2.0
 */
public class EBRIM_Mapping {
    ...
    /**
     * A very important method, it allows the retrieval of the original dom tree from the xslt-processor, which in it's
     * turn can be used to find the prefix mappings used in the document.
     * 
     * @param context
     *            the xslt Context, which will be given by the xalan processor
     * @param extElem
     *            a class encapsulating some calling parameters from the xslt script, it is not used in this class.
     */
    ...
    org.apache.xalan.templates.ElemExtensionCall extElem ) {
        ...
        if ( LOG.getLevel() == ILogger.LOG_DEBUG ) {
            ...
            if ( ns != null ) {
                LOG.logDebug( " For the 'rim:' prefix we found following ns: "
                              + ns.toASCIIString() );
            } else {
    ...
    public EBRIM_Mapping() {
        try {
            InputStream is = EBRIM_Mapping.class.getResourceAsStream( "adv_catalog.properties" );
    ...
    public String mapPropertyPath( Node node )
        ...
        LOG.logDebug( " The supplied xml-Node results into following (normalized) propertypath: "
                      + propertyNode );
        LOG.logDebug( " EBRIMG_Mappign#mapPropertyPath: We have got the variables: "
                      + variables.toString() );
            ...
            if ( propertyName != null ) {
                ...
                if ( propertyName.startsWith( "$" ) ) {
                    ...
                    if ( !variables.containsKey( propertyName.substring( 1 ) )
                        ...
                        LOG.logDebug( " trying to find mapping for a property: "
                                      + propertyName + ", with found variable: " + tmpVarReference );
                    ...
                    if ( previousMapping == null ) {
                        ...
                        // @status=app:status
                        LOG.logDebug( " trying to find mapping for attribute: "
                                      + propertyName );
                        mappedName = mapping.getProperty( propertyName );
                    ...
                    LOG.logDebug( " Trying to find mapping for simple propertyName: "
                                  + propertyName );
                ...
                if ( mappedName == null ) {
                    ...
                    if ( ( "rim:Value".equals( propertyName ) && ( "rim:ValueList".equals( previousMapping ) || "wrs:ValueList".equals( previousMapping ) ) )
                         || ( "wrs:AnyValue".equals( propertyName ) && "wrs:ValueList".equals( previousMapping ) ) ) {
    ...
    public String mapTypeNames( Node node )
        ...
        LOG.logDebug( " EBRIM_Mapping#mapTypeNames: We have got the variables: "
                      + variables.toString() );
        ...
        for ( QualifiedName qName : typeNames ) {
            ...
            if ( ns == null || rimNS.equals( ns.toString() ) ) {
                LOG.logDebug( " We found the following namespace for the ElementSetName/@typeName: "
                              + ns + " so we map to the prefix rim." );
                prefix = "rim:";
            }
    ...
    public NodeList mapElementSetName( Node elementSetNameNode ) {
        ...
        try {
            ...
            if ( elementSetName != null ) {
                // Setting the node which will be used to find the prefixes.
                Node prefixResolverNode = getPrefixResolverNode( elementSetNameNode );
                String typeNamesAttribute = XMLTools.getNodeAsString( elementSetNameNode, "@typeNames", nsContext, null );
                Element queryElement = (Element) elementSetNameNode.getParentNode();
                ...
                if ( typeNamesAttribute != null ) {
                    ...
                    if ( firstOriginalFilterNode != null ) {
                        LOG.logDebug( " The ogc:Filter has a firstChild node, therefore                   creatining an extra ogc:And." );
                        Element a = queryElement.getOwnerDocument().createElementNS(
                                                                                     CommonNamespaces.OGCNS.toASCIIString(),
                                                                                     "ogc:And" );
                        queryElement.getOwnerDocument().importNode( a, false );
                    ...
                    for ( int i = 0; i < typeNames.size(); ++i ) {
                        ...
                        String result = mapping.getProperty( prefix + qName.getLocalName() );
                        LOG.logDebug( " for the FeatureType: " + prefix
                                      + qName.getLocalName() + " we found following mapping: " + result );
                        if ( result != null ) {
                            if ( !"app:RegistryObject".equals( result ) ) {
                                ...
                                LOG.logDebug( " in elementSetName found alias: "
                                              + aliasPrefix + " for the typename: " + qName.getLocalName() );
        ...
        for ( int i = 0; i < nodeList.getLength(); ++i ) {
            LOG.logDebug( " Node " + i + " has a localName: "
                          + nodeList.item( i ).getLocalName() );
        }
    ...
    private List<QualifiedName> parseTypeList( Node node, String typeNamesAttribute, Map<String, QualifiedName> vars ) {
        ...
        for ( String s : splitter ) {
            try {
                LOG.logDebug( " Trying to parse following typeName (with/without variables): "
                              + s );
                if ( !s.startsWith( "$" ) ) {
     ...
     *            the value of the ElementSetName node (one of full ,brief, summary)
