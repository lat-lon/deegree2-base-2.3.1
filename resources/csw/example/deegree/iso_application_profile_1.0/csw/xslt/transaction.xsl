...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="gmd:CI_Contact">
		<app:CI_Contact>
			<xsl:variable name="voice" select="gmd:phone/gmd:CI_Telephone/gmd:voice/gco:CharacterString" />
			<xsl:if test="$voice != ''">
				<app:voice>
					<xsl:value-of select="$voice" />
				</app:voice>
			</xsl:if>
			<xsl:variable name="fax" select="gmd:phone/gmd:CI_Telephone/gmd:facsimile/gco:CharacterString" />
			<xsl:if test="$fax != ''">
				<app:facsimile>
	...
	<xsl:template match="gmd:CI_Citation">
		...
		<app:CI_Citation>
			...
			<xsl:variable name="identifier" select="gmd:identifier/gmd:MD_Identifier/gmd:code/gco:CharacterString" />
	...
	<xsl:template match="gmd:MD_Usage">
		<app:MD_Usage>
			...
			<xsl:if test="gmd:userContactInfo/gmd:CI_ResponsibleParty">
				<app:RespParty>
	...
	<xsl:template match="gmd:MD_LegalConstraints">
		<app:MD_LegalConstraints>
			...
			<xsl:variable name="otherConst" select="gmd:otherConstraints/gco:CharacterString" />
			<xsl:if test="$otherConst != ''">
				<app:otherConstraints>
					<xsl:value-of select="$otherConst" />
				</app:otherConstraints>
			</xsl:if>
			<xsl:for-each select="gmd:useConstraints/gco:MD_RestrictionCode">
				<app:useConstraints>
	...
	<xsl:template match="gmd:distributionFormat">
		<app:distributionFormat>
			<app:MD_Format>
				<xsl:variable name="Formatname" select="gmd:MD_Format/gmd:name/gco:CharacterString" />
				<xsl:if test="boolean( $Formatname != '' )">
					<app:name>
						<xsl:value-of select="$Formatname" />
					</app:name>
				</xsl:if>
				<xsl:variable name="Formatversion" select="gmd:MD_Format/gmd:version/gco:CharacterString" />
	...
	<xsl:template match="gmd:MD_DigitalTransferOptions">
		<app:transferOptions>
			<app:MD_DigTransferOpt>
				...
				<xsl:if test="boolean( gmd:onLine )">
					<app:onlineResource>
