...
<xsl:stylesheet version="1.0" 
	...
	xmlns:java="java"	xmlns:minmax="org.deegree.framework.xml.MinMaxExtractor" xmlns:mapping="org.deegree.ogcwebservices.csw.iso_profile.Mapping2_0_2" exclude-result-prefixes="false" >
	<!-- ================================================================== -->
	...
	<xsl:template name="summary_distributioninfo">
		<gmd:distributionInfo>
			<gmd:MD_Distribution>
				<xsl:for-each select="app:distributionFormat/app:MD_Format">
				...
				<xsl:for-each select="app:transferOptions">
					<gmd:transferOptions>
						<gmd:MD_DigitalTransferOptions>
							<xsl:if test="app:MD_DigTransferOpt/app:onlineResource/app:CI_OnlineResource/app:linkage">
								<gmd:onLine>
									<gmd:CI_OnlineResource>
										<gmd:linkage>
											<gmd:URL>
												<xsl:value-of select="app:MD_DigTransferOpt/app:onlineResource/app:CI_OnlineResource/app:linkage" />
											</gmd:URL>
										</gmd:linkage>
									</gmd:CI_OnlineResource>																		
								</gmd:onLine>
	...
	<xsl:template match="app:LI_ProcessStep">
		<gmd:LI_ProcessStep>
			...
			<xsl:if test="app:processor">
				<gmd:processor>
					<xsl:apply-templates select="app:processor" />
				</gmd:processor>
			</xsl:if>
		</gmd:LI_ProcessStep>
	</xsl:template>
