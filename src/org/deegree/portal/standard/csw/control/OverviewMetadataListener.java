//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/csw/control/OverviewMetadataListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class OverviewMetadataListener extends SimpleSearchListener {
    ...
    public void actionPerformed( FormEvent event ) {
        ...
        if ( params == null || params.length == 0 ) {
            ...
            try {
                ...
                gotoErrorPage( Messages.getMessage( "IGEO_STD_CSW_INVALID_RPC_EVENT",
                                                    e.getMessage() ) );
                LOG.logError( e.getMessage() );
                return;
            }
    ...
    protected void validateRequest( RPCWebEvent rpcEvent )
        ...
        if ( params.length != 1 ) {
            throw new CatalogClientException( Messages.getMessage( "IGEO_STD_CSW_WRONG_PARAMS_NUMBER",
                                                                   "1", params.length ) );
        }
        ...
        if ( !containsCatalog ) {
            throw new CatalogClientException( Messages.getMessage( "IGEO_STD_CSW_WRONG_CAT",
                                                                   rpcCatalog ) );
        }
        ...
        if ( !formats.contains( rpcFormat ) ) {
            throw new CatalogClientException( Messages.getMessage( "IGEO_STD_CSW_WRONG_FORMAT",
                                                                   rpcCatalog, rpcFormat ) );
        }
        ...
        if ( !protocols.contains( rpcProtocol ) ) {
            throw new CatalogClientException( Messages.getMessage( "IGEO_STD_CSW_WRONG_PROTOCOL",
                                                                   rpcCatalog, rpcProtocol ) );
        }
     ...
     * 
     * @param catalogsMap
     *            The Map containing the data title (as key) and the List of available service
     *            catalogues names (as value)
     * @param title
     *            The key for the value in the passed Map.
     * @return Returns an Array of Strings for the available service catalogues. If no service
     *         catalogues are available, an Array of one empty String is returned.
     */
