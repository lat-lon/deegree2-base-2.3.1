//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/csw/control/ISO19115RequestFactory.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class ISO19115RequestFactory extends CSWRequestFactory {
    ...
    private String replaceVarsInSearchRequest( String request, String resultType )
        ...
        // For 2.0.2
        String queryType = "gmd:MD_Metadata";
        /*
    ...
    private String handleMetadataType() {
        ...
        if ( ( struct.getMember( RPC_MDTYPE ) ) != null ) {
            ...
            if ( mdTypes != null && mdTypes.length > 0 ) {
                if ( mdTypes.length > 1 && mdTypes.length < 4 ) {
                    sb.append( "<ogc:Or>" );
                    ...
                    // This is the case, where the four metadata types are searched and in this case, no type filter
    ...
    private String handleBbox()
        ...
        if ( struct.getMember( Constants.RPC_BBOX ) != null ) {
            ...
            try {
                ...
                System.out.println( "srs: " + config.getSrs() );
