//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/csw/control/DisplayMapListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class DisplayMapListener extends AddToShoppingCartListener {
     ...
     * @param rpcEvent
     *            The RpcWebEvent contains the parameters defining the DataSessionRecords to get
     *            from the passed List.
     * @return Returns a sublist of the passed contents, where each of the contained
     *         DataSessionRecords is defined by the parameters of the passed rpcEvent OR null, if no
     *         matches were found in the passed List.
     * @throws CatalogClientException
     * @throws IllegalArgumentException,
     *             if if at least one record defined by the passed rpcEvent is not part of the
     *             passed List.
     */
     ...
     * as value. Both titles and addresses are taken from the passed List of DataSessionRecords. For
     * each DataSessionRecord only the first WMS address is used.
     * 
     * @param dataSessionRecords
    ...
    /**
     * Creates a HashMap with the WebMapService address as key and a WMSCapabilities object as
     * value.
     * 
     * @param addressRequestMap
     *            The Map containing the address as key for the Map to be returned, and the request
     *            as String, with which to create the Capability object.
     * @return Returns a HashMap containing the WMS address as key and a WMSCapabilities object as
     *         value.
    ...
    /**
     * Gets the identifier that is part of the same DataSessionRecord in the passed list as the
     * passed title.
     * 
     * @param dataSessionRecords
     *            List of DataSessionRecords
    ...
    /**
     * Recursion over all layers to find the layer that matches the submitted title. If no layer can
    ...
    /**
     * Extracts the context name from the RPCStruct of the first parameter in the passed
     * RPCWebEvent.
     * 
     * @param rpcEvent
     ...
     * 
     * If the session ID equals "null", then the default user name is returned. If no user is
     * assigned to the session ID a CatalogClientException will be returned. If the session is
     * closed or expired an exception will be thrown.
     * 
     * @param sessionId
     ...
     * combining bounding box. If none of the passed records has a bounding box, then null is
     * returned.
     * 
     * @param rpcDataSessionRecords
     *            List of DataSessionRecords for which to determin a combining bounding box.
     * @return Returns a new GM_Envelope for a combining bounding box, OR null, if no bounding box
     *         was set in any of the passed DataSessionRecords.
     * @throws UnknownCRSException
     */
