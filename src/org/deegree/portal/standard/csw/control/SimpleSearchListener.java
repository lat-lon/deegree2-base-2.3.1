// $HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/csw/control/SimpleSearchListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * A <code>${type_name}</code> class.<br/> This listener does more than just search for data. It searches for data
 * *and* then searches if there are services (WMS, WFS) available, which provide this data.
 * 
 * @author <a href="mailto:mays@lat-lon.de">Judit Mays</a>
 * @author last edited by: $Author: melmasry $
 * 
 * @version $Revision: 14742 $, $Date: 2008-11-06 11:43:55 +0100 (Do, 06. Nov 2008) $
 */
        ...
        for ( int i = 0; i < catalogs.size(); i++ ) {
            ...
            try {
                ...
                if ( LOG.getLevel() == ILogger.LOG_DEBUG ) {
                    LOG.logDebug( "\nResponse for perform request is: \n",
        ...
        while ( iterator.hasNext() ) {
            ...
            try {
                for ( int i = 0; i < nl.size(); i++ ) {
                    titles.add( extractValue( (Node) nl.get( i ), getXPathToTitle( (Element) nl.get( i ) ) ) );
                }
            } catch ( Exception e ) {
