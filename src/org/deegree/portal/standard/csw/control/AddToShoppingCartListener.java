//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/csw/control/AddToShoppingCartListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class AddToShoppingCartListener extends SimpleSearchListener {
    ...
    public void actionPerformed( FormEvent event ) {
        ...
        try {
            ...
            gotoErrorPage( Messages.getMessage( "IGEO_STD_CSW_INVALID_SERVICE_REQ",
                                                e.getMessage() ) );
            LOG.logError( e.getMessage(), e );
            return;
        }
        ...
        try {
            ...
            gotoErrorPage( Messages.getMessage( "IGEO_STD_CSW_SERVER_ERROR",
                                                e.getMessage() ) );
            LOG.logError( e.getMessage(), e );
            return;
        }
        ...
        try {
            ...
            gotoErrorPage( Messages.getMessage( "IGEO_STD_CSW_FAIL_CREATE_SSR",
                                                e.getMessage() ) );
            LOG.logError( e.getMessage(), e );
            return;
        }
        ...
        DataSessionRecord dsr = getDataSessionRecordFromList( dsrList, dataId, dataCatalog,
                                                              dataTitle );
        cart.add( dsr );
        session.setAttribute( Constants.SESSION_SHOPPINGCART, cart );

        getRequest().setAttribute( Constants.MESSAGE,
                                   Messages.getMessage( "IGEO_STD_CSW_SUCCESS_ADD_DATASET" ) );
        ...
        while ( it.hasNext() ) {
            ...
            for ( int j = 0; j < mdList.size(); j++ ) {
                ...
                try {
                    ...
                    serviceTypeVersion = extractValue( mdNode,
                                                       config.getXPathToServiceTypeVersion() );
                } catch ( Exception e ) {
                    throw new CatalogClientException( Messages.getMessage( "IGEO_STD_CSW_ERROR_EXTRACT_VAL", 
                                                                           e.getMessage() ) );
                }

                ServiceSessionRecord ssr = new ServiceSessionRecord( id, catalog, title, address,
                                                                     serviceType,
                                                                     serviceTypeVersion );
                ssrList.add( ssr );
            }
     ...
     */
    private List addServiceSessionRecords( List dataSessionRecList, String dataId,
    ...
    protected DataSessionRecord getDataSessionRecordFromList( List dataSessionRecList,
                                                             String dataId, String dataCatalog,
                                                             String dataTitle ) {
