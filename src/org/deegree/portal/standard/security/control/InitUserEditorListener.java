//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/security/control/InitUserEditorListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class InitUserEditorListener extends AbstractListener {
    ...
    public void actionPerformed( FormEvent event ) {
        ...
        try {
            ...
            if ( configStartContexts != null && configStartContexts.length() != 0 ) {
                ...
                if ( getHomePath().endsWith( "/" ) ) {
                    confFilePath = getHomePath() + configStartContexts;
                } else {
                    confFilePath = getHomePath() + '/' + configStartContexts;
                }
                */
                ...
                if ( absFilePath.exists() ) {
                    ...
                    Map<String, StartContext> contextsList = parseConetxtsXml( fragment.getRootElement() );
     ...
     * 
     * @param rootElem
     * @return a [2] map Array that contains a list of the read contexts as a first element and a
     *         list of the user contexts as a second element.the array lengh should always be 2
     * @throws XMLParsingException
     * @throws ClientConfigurationException
     * @throws IOException
     * @throws DOMException
     */
    private Map<String, StartContext> parseConetxtsXml( Element rootElem )
        ...
        List list = XMLTools.getRequiredNodes( rootElem, xPath, cn );
     ...
     * @param rootElem
     * @param contextsHT
     *            its the hashtable containing all the context read from the start_config.xml They
     *            will be useful to determine the context name
...
//                            reader.close();
    ...
    /**
     * @param reader
     *            The reader of the file where we should search
     * @param target
    ...
    private void updateDefaultContext( String contextName, String contextPath, Element rootElem ) {
        ...
        try {
            ...
            File defaultContextFile = new File( getHomePath() + confPath + getUsersDirectory( rootElem ) + "/context.properties" );
            ...
            if ( pathChanged ) {
                BufferedWriter bufferedWriter = new BufferedWriter( new FileWriter( defaultContextFile.getCanonicalPath() ) );
                ...
                getRequest().setAttribute( "DEFAULTCHANGED",
                                           contextFile.getCanonicalPath().substring( contextFile.getCanonicalPath().indexOf( "WEB-INF" ) ) );
            }
