//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/security/control/StoreRightsListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class StoreRightsListener extends AbstractListener {
    ...
    private void setAccessRightsForLayers( int[] layers, Map[] layerConstraints, SecurityTransaction transaction,
                                           Role role, SecuredObject[] presentLayers )
        ...
        for ( int i = 0; i < presentLayers.length; i++ ) {
            ...
            if ( isAccessible ) {
                ...
                Right[] rights = new Right[] { new Right( layer, RightType.GETMAP, filter ),
    ...
    private void extractLayerValues( int[] layers, Map<String, Object>[] layerConstraints, RPCParameter[] layerParams )
                            throws RPCException {
        for ( int i = 0; i < layerParams.length; i++ ) {
            ...
            if ( layerParams[i].getValue() instanceof RPCParameter[] ) {
                ...
                RPCParameter[] constrainParams = (RPCParameter[]) layerParams[i].getValue();
                try {
                ...
                try {
                    layers[i] = Integer.parseInt( (String) layerParams[i].getValue() );
                } catch ( NumberFormatException e ) {
                    String s = Messages.getMessage( "IGEO_STD_STORERIGHTS_INT_EXPECTED" );
                    throw new RPCException( s );
                }
...
//        }
    ...
    /**
     * Builds a filter encoding-expression as a constraint for GetMap-operations from the values
     * stored in the given <code>Map</code>.
     * 
     * @param constraintMap
     * @return String
     * @throws RPCException
