//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/sos/control/GetObservationListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class GetObservationListener extends AbstractSOSListener {
    ...
    protected String createRequest( RPCMethodCall mc )
        ...
        sb.append( "xmlns:gml='http://www.opengis.net/gml' xmlns:ogc='http://www.opengis.net/ogc' " );
    ...
    protected Object createData( RPCMethodCall mc, HashMap<String,Document> map )
        ...
        List<Observation> obsList = new ArrayList<Observation>( 100 );
        ...
        try {
            ...
            while ( iterator.hasNext() ) {
                ...
                List<Node> obsNodes = XMLTools.getNodes( e,
                                                   "/om:ObservationCollection/om:observationMembers/om:ObservationArray"
                                                                           + "/om:observationMembers/gml:Observation",
                                                   nsContext );

                for ( int i = 0; i < obsNodes.size(); i++ ) {

                    String timeStamp = XMLTools.getNodeAsString( obsNodes.get( i ),
                                                                 "gml:timeStamp/gml:TimeInstant/gml:timePosition",
                                                                 nsContext, null );

                    String value = XMLTools.getNodeAsString( obsNodes.get( i ), "gml:resultOf/gml:QuantityList",
                                                             nsContext, null );
                ...
                Observation[] observations = obsList.toArray( new Observation[obsList.size()] );
