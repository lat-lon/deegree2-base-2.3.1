//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/sos/control/DescribeSensorListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class DescribeSensorListener extends AbstractSOSListener {
    ...
    protected void validateRequest( RPCMethodCall mc )
        ...
        if ( struct.getMember( Constants.TYPENAME ) == null || "".equals( Constants.TYPENAME ) ) {
            throw new SOSClientException( "TypeName parameter must be set "
                                          + "to perform a Sensor Observation Service "
    ...
    protected Object createData( RPCMethodCall mc, HashMap<String, Document> map )
        ...
        try {
            ...
            while ( iterator.hasNext() ) {
                ...
                Element e = map.get( key ).getDocumentElement();

                String[] ids = XMLTools.getNodesAsStrings(
                                                           e,
                                                           "/sml:Sensors/sml:Sensor/sml:identifiedAs/sml:Identifier",
                                                           nsContext );
                ...
                locatedUsing[0] = new GeoPositionModel( null, null, null, null,
                                                        new EngineeringCRS( srcSRS ),
                                                        new GeographicCRS( refSRS ), new Object[0] );
                ...
                for ( int l = 0; l < productList.getLength(); l++ ) {
                    ...
                    String[] prodIds = XMLTools.getNodesAsStrings(
                                                                   e,
                    ...
                    String observableName = XMLTools.getNodeAsString(
                                                                      e,
                    ...
                    measures[l] = new Product( productIdentifiedAs, productClassifiedAs,
                                               productDescription, productLocatedUsing,
                                               productDerivedFrom, productHasCRS,
                                               productObservable, productId );

                }

                sensorDescriptions[i] = new SensorMetadata( identifiedAs, classifiedAs, hasCRS,
                                                            locatedUsing, describedBy, attachedTo,
                                                            measures );
                i++;
            }

        } catch ( Exception e ) {
            LOG.logError( "Error creating sensor descriptions: " + e.getMessage() );
