//$HeadURL: svn+ssh://melmasry@svn.wald.intevation.org/deegree/base/trunk/src/org/deegree/portal/standard/context/control/FullScreenListener.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * Convert the current view context to a full screen mode. It hides all the modules except the map
 * and the toolbar. It also keeps whatever changes were made in the original context to the full
 * screen context
 * 
...
public class FullScreenListener extends AbstractContextListener {
     ...
     * files, under <code>${context-home}/WEB-INF/conf/igeoportal/</code>.
     */
    ...
    /**
     * ValidateRPC looks in the RPCStruct for all needed elements and gotoErrorPage in case an
     * element is found or wronglz formated, this is useful so that we won't need to check later for
     * anz variables, simply get them and start Working
     * 
     * @param rpc
    ...
    /**
     * This method checks the context.properties file to make sure that all fields needed are
     * available
     * 
     */
    ...
    /**
     * Applies the modules read from the context.properties file to the current context using the
     * data from the session on the defaultContext
     * 
     * @param rpc
     ...
     * 
     * @param vc the Viewcontext after convert
     * @throws ParserConfigurationException
     ...
     * 
     * @param struct the rpc struct containing the current bbox envelope
     * @param vc view context with the layers
