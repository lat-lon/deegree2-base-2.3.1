//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/graphics/displayelements/LabelFactory.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * <p>
 * Different geometry-types (of the LabelDisplayElement) imply different strategies concerning the
 * way the <tt>Labels</tt> are generated.
 * <p>
 * 
 * @author <a href="mailto:mschneider@lat-lon.de">Markus Schneider </a>
 * @version $Revision: 10615 $ $Date: 2008-03-18 13:42:49 +0100 (Tue, 18 Mar 2008) $
 */
public class LabelFactory {
    ...
    /**
     * Determines positions on the given <tt>MultiCurve</tt> where a caption could be drawn. For
     * each of this positons, three candidates are produced; one on the line, one above of it and
     * one below.
     * <p>
     * 
     * @param multiCurve
    ...
    /**
     * Determines positions on the given <tt>Curve</tt> where a caption could be drawn. For each
     * of this positons, three candidates are produced; one on the line, one above of it and one
     * below.
     * <p>
     * 
     * @param curve
            ...
            if ( getDistance( boxStartX, boxStartY, x, y ) >= w ) {
                ...
                case LinePlacement.TYPE_ABSOLUTE: {
                    label = createLabel( caption, font, sldFont.getColor( feature ), metrics, feature,
                                         ...
                                         rotation, 0.0, 0.5, ( w - width ) / 2, perpendicularOffset );
                    break;
                }
                case LinePlacement.TYPE_ABOVE: {
                    label = createLabel( caption, font, sldFont.getColor( feature ), metrics, feature,
                                         symbolizer.getHalo(), boxStartX, boxStartY, (int) width, (int) height,
                                         rotation, 0.0, 0.5, ( w - width ) / 2, delta + deviation[0] );
                    break;
                }
                ...
                case LinePlacement.TYPE_AUTO: {
                    label = createLabel( caption, font, sldFont.getColor( feature ), metrics, feature,
                                         symbolizer.getHalo(), boxStartX, boxStartY, (int) width, (int) height,
                                         rotation, 0.0, 0.5, ( w - width ) / 2, -delta - deviation[1] );
                    break;
                }
