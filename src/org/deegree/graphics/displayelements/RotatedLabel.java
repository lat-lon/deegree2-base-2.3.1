//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/graphics/displayelements/RotatedLabel.java $
/*----------------    FILE HEADER  ------------------------------------------
...
class RotatedLabel implements Label {
    ...
    RotatedLabel( String caption, Font font, Color color, LineMetrics metrics, Feature feature, Halo halo, int x,
                  int y, int w, int h, double rotation, double anchorPoint[], double[] displacement ) {
        ...
        for ( int i = 0; i < 4; i++ ) {
            int[] point = transformPoint( xpoints[i], ypoints[i], tx, ty, rotation, displacement[0], displacement[1] );
    ...
    private int[] transformPoint( int x, int y, int tx, int ty, double rotation, double displacementX,
