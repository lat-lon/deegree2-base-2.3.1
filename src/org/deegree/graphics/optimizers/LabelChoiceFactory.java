//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/graphics/optimizers/LabelChoiceFactory.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class LabelChoiceFactory {
            ...
            if ( geometry instanceof Point ) {
                ...
                labels[0] = LabelFactory.createLabel( caption, font, sldFont.getColor( feature ), metrics, feature,
                                                      symbolizer.getHalo(), x, y, w, h, rotation, new double[] { 0.0,
                                                                                                                0.0 },
                                                      new double[] { displacement[0], displacement[1] } );
                ...
                if ( intersection != null && intersection.getCentroid() != null ) {
                    ...
                    labels[0] = LabelFactory.createLabel( caption, font, sldFont.getColor( feature ), metrics, feature,
                                                          ...
                                                          new double[] { 0.5, 0.5 }, new double[] { 0, 0 } );
                    labels[1] = LabelFactory.createLabel( caption, font, sldFont.getColor( feature ), metrics, feature,
                                                          symbolizer.getHalo(), x, y, w, h, rotation,
                                                          new double[] { 0.5, 0.0 }, new double[] { 0, 0 } );
                    labels[2] = LabelFactory.createLabel( caption, font, sldFont.getColor( feature ), metrics, feature,
                                                          symbolizer.getHalo(), x, y, w, h, rotation,
                                                          new double[] { 0.5, 1.0 }, new double[] { 0, 0 } );
     ...
     * <tt>Label</tt> could be drawn. For each <tt>LabelChoice</tt>, three candidates are
     * generated: one on the line, one above it and one below.
     * <p>
     * 
     * @param element
     * @param g
    ...
    /**
     * Determines <tt>LabelChoice</tt>s for the given <tt>Curve</tt> where a <tt>Label</tt>
     * could be drawn. For each <tt>LabelChoice</tt>, three candidates are generated: one on the
     * line, one above it and one below.
     * <p>
     * 
     * @param curve
     * @param element
