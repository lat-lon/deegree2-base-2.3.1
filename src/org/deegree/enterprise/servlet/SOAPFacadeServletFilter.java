//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/enterprise/servlet/SOAPFacadeServletFilter.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * 
 * @version $Revision: 10766 $
 * @author <a href="mailto:poth@lat-lon.de">Andreas Poth</a>
 * @author last edited by: $Author: apoth $
 * 
 * @version 1.0. $Revision: 10766 $, $Date: 2008-03-28 10:52:47 +0100 (Fri, 28 Mar 2008) $
 * 
 */
public class SOAPFacadeServletFilter implements Filter {
    ...
    /**
     * returns true if the passed SOAP message contains a header that must be understood by a
     * handling node
     * 
     * @param xmlReq
     * @return true if the passed SOAP message contains a header that must be understood by a
     *         handling node
     * @throws XMLParsingException
     */
     ...
     * 
     * NamedNodeMap nnm = node.getAttributes(); for (int i = 0; i < nnm.getLength(); i++) { Node nd =
     * nnm.item( i ); if ( node.getNodeName().startsWith( "xmlns" ) ) { xmlns.put(
     * node.getNodeName(), nd.getNodeValue() ); } } NodeList nl = node.getChildNodes(); for (int i =
     * 0; i < nl.getLength(); i++) { if ( nl.item( i ) instanceof Element ) { xmlns =
     * collectNamespaces( (Element) nl.item( i ), xmlns ); } }
     * 
     * return xmlns; }
     */
