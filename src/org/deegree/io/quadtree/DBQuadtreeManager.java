//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/quadtree/DBQuadtreeManager.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class DBQuadtreeManager<T> {
     ...
     * @param type
     *            the type of the id of the quadtree, valid are Types.INTEGER and Types.VARCHAR. if
     *            unsure use Intger.MIN_VALUE and call {@link #determineQuattreeType()}, to get an
     ...
     * @throws IndexException if the type information could not be retrieved either because no connection was acquired or an error occurred while executing the select statement.
     */
            ...
            try {
                ...
                if ( tableID != null ) {
                    ...
                    LOG.logError( "Could not find the foreign key (fk_root) of the table: '" + table
                                  + "' is your database is set up correct?" );
                }
            } catch ( SQLException e ) {
                LOG.logError(
     ...
     * @param id
     *            of the feature to store in the database, currently String and Integer are
     *            supported. If it is neither, the Object is saved as an object, which may result in
     *            inconsitencies.
     * @param jdbc
    ...
    /**
     * Inserts a row into the quadtree meta data structure 'TAB_DEEGREE_IDX', containing information
     * on the table, geometry, indexname, owner and the foreign_key to the index table.
     * 
     * @param fk_indexTree
     * @throws IndexException
     */
    ...
    /**
     * Creates actually two tables, an indextable, which will hold the actual quadtree and an
     * index_item table which is a join-table between the dbNodes and the feature_ids.
     * 
     * @param indexTable
