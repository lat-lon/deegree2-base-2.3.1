//$HeadURL: svn+ssh://mschneider@svn.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/datastore/FeatureId.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * Keeps track of all persistent features that are in a locked state, i.e. a {@link LockFeature}
 * request has been issued to lock them.
 * <p>
 * Locked features cannot be updated or deleted except by transactions that specify the appropriate
 * lock identifier.
 * <p>
 * The <code>LockManager</code> also ensures that active locks survive a restart of the VM -
 * therefore it keeps serialized and up-to-date versions of all active {@link Lock} instances in a
 * temporary directory. The directory is specified by the <code>java.io.tmpdir</code> system
 * property. On first initialization, i.e. the first call to {@link #getInstance()}, the directory
 * is scanned for all files matching the pattern <code>deegree-lock*.tmp</code>, and these are
 * deserialized to rebuild the <code>LockManager</code>'s status.
 * 
...
public class LockManager {
    ...
    static String FILE_PREFIX = "deegree-lock";

    static String FILE_SUFFIX = ".tmp";
    ...
    /**
     * Acquires a lock for the given {@link LockFeature} request. The affected feature instances and
     * their descendant features + super features have to be specified as well.
     * <p>
     * If the lockAction in the request is set to ALL and not all requested features could be
     * locked, a {@link DatastoreException} will be thrown.
     * <p>
     * If no features have been locked at all, a lock will be issued, but the lock is not registered
     * (as requested by the WFS spec.).
     * 
     * @param request
    ...
    /**
     * Releases the specified lock completely (all associated features are unlocked) and removes it
     * (also from the temporary directory).
     * 
     * @param lockId
     *            lock identifier
     * @throws DatastoreException
    ...
    /**
     * Releases the given lock completely (all associated features are unlocked) and removes it
     * (also from the temporary directory).
     * 
     * @param lock
     *            lock to be released
     ...
     * <li>If the lock is not empty and it has not been stored yet, it is written to a temporary
     * file.</li>
     * <li>If the lock is not empty and it has already been stored, the existing file is
     * overwritten with the current lock status.</li>
     * </ul>
     * 
     * @param lock
