//$HeadURL: svn+ssh://developername@svn.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/datastore/memory/MemoryWFSDatastore.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class CachedWFSDatastore extends Datastore {
    ...
    private void init()
        ...
        LOG.logInfo( "use cache size: " + cacheSize );
        if ( mconf != null ) {
            synchronized ( mconf ) {
                ...
                if ( rTree == null ) {
                    ...
                    try {
                        ...
                        int k = 1;
                        do {
                            String s = StringTools.concat( 100, "reading feature: " , k , " - " , (k+1000) );
                            LOG.logInfo( s );  
                            ...
                            for ( int i = 0; i < fc.size(); i++ ) {
                                ...
                                insertIntoRTree( rTree, feature, i );
                            }
                            k += 1000;
                        } while ( fc.size() == 1000 );
                        LOG.logInfo( Integer.toString( fc.size() ), " features loaded" );
                    } catch ( Exception e ) {
                        LOG.logError( e.getMessage(), e );
                        throw new DatastoreException( e.getMessage(), e );
                    }
