//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/datastore/sql/wherebuilder/WhereBuilder.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class WhereBuilder {
    ...
    /**
     * Appends the alias-qualified, comma separated list of all tables to be joined in order to
     ...
     * <ul>
     * <li>left outer joins are generated for each join that is necessary, because of filter
     * expressions that target properties stored in related tables (condition joins)
     * <li>cross-product joins are generated for all feature type root tables (feature type joins)
     * that have not joined by filter expression joins before</li>
     * </ul>
     * 
     * @param query
    ...
    /**
     * Appends the alias-qualified, comma separated list of tables to be joined (for one root
     * feature type node). This includes the join conditions (necessary for the filter conditions),
     * which are generated in ANSI-SQL left outer join style.
     * 
     * @param query
     *            the list is appended to this <code>SQLStatement</code>
     * @param rootNode
    ...
    protected void appendFeatureFilterAsSQL( StatementBuffer query, FeatureFilter filter )
        ...
        MappingField mapping = rootFt.getGMLId().getIdFields()[0];
        query.append( ' ' );
