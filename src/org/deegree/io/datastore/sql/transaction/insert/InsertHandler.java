//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/datastore/sql/transaction/insert/InsertHandler.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class InsertHandler extends AbstractRequestHandler {
    ...
    /**
     * Builds the <code>InsertRow</code>s that are necessary to insert the given property
     * instance (including all it's subfeatures).
     * 
     * @param property
    ...
    private void insertProperty( FeatureProperty property, MappedPropertyType propertyType, InsertRow featureRow )
        ...
        if ( propertyType instanceof SimplePropertyType ) {
            LOG.logDebug( "- Simple property '" + propertyType.getName() + "', value='" + getPropertyValue( property )
                          + "'." );
    ...
    private void insertProperty( MappedGeometryPropertyType pt, FeatureProperty property, InsertRow featureRow )
        ...
        if ( deegreeGeometry.getCoordinateSystem() == null ) {
            ...
            String msg = "Insert-Transformation: geometry srs: " + deegreeGeometry.getCoordinateSystem().getIdentifier()
                         + " -> property srs: " + pt.getSRS();
            LOG.logDebug( msg );
            if ( createSrsCode == -1 ) {
    ...
    private void insertProperty( MappedFeaturePropertyType pt, FeatureProperty property, InsertRow featureRow )
        ...
        if ( relations.length == 1 ) {
            if ( relations[0].isFromFK() ) {
                ...
                if ( pkFields[0] != null ) {
                    LOG.logDebug( "Getting column " + pkFields[0].getField() + "from table: " + pkFields[0].getTable()
                                  + " of the featureRow: " + featureRow.getTable() );
                }
                ...
                if ( pkField == null ) {
                    ...
                    if ( pkFields[0] != null ) {
                        ...
                        if ( relations[0] != null ) {
                            ...
                            msg = Messages.getMessage( "DATASTORE_NO_FK_VALUE",
                                                       "unknown primary keys in 'from'-fields", "unknown 'from'-table" );
                        }
                    }
                ...
                if ( !cutLink ) {
                    ...
                    subFeatureRow.setColumn( fkFields[0].getField(), pkField.getValue(), pkField.getSQLType(), false );
                }
            }
            ...
            if ( relations[1].isFromFK() ) {
                ...
                if ( pkField == null ) {
                    throw new TransactionException( "Insertion of feature property using join table failed: "
                                                    + "no value for join table key column '" + pkField.getColumnName()
                                                    + "'." );
                }
                if ( !cutLink ) {
