//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/datastore/sql/transaction/UpdateHandler.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version $Revision: 13847 $, $Date: 2008-09-08 14:42:21 +0200 (Mo, 08. Sep 2008) $
 */
public class UpdateHandler extends AbstractRequestHandler {
     ...
     */
    public int performUpdate( MappedFeatureType ft, Map<PropertyPath, FeatureProperty> replacementProps,
                              Filter filter )
                            throws DatastoreException {
     ...
     * Performs an update operation against the associated datastore.
     * <p>
     * The filter must match exactly one feature instance (or none) which is then replaced by the
     * specified replacement feature.
     * 
    ...
    public int performUpdate( MappedFeatureType mappedFeatureType, Feature replacementFeature,
        ...
        List<FeatureId> fids = determineAffectedAndModifiableFIDs( mappedFeatureType, filter,
                                                                   this.lockId );
     ...
     * <p>
     * If the selected property is a direct property of the feature, the root feature is updated,
     * otherwise the targeted subfeatures have to be determined first.
     * 
     * @param fid
     * @param ft
     * @param propertyName
    ...
    /**
     * Returns the relations (the "path") that lead from the feature type's table to the subfeature
     * table which is targeted by the specified property name.
     * 
     * @param ft
     *            source feature type
     * @param path
        ...
        if ( pt instanceof MappedSimplePropertyType ) {
            ...
            if ( content.isUpdateable() ) {
                if ( content instanceof MappingField ) {
                    updateSimpleProperty( fid, tablePath, (MappingField) content, replacementValue );
                }
            } else {
            ...
            if ( deegreeGeometry.getCoordinateSystem() == null ) {
                ...
                String msg = "Insert-Transformation: geometry srs: "
                             + deegreeGeometry.getCoordinateSystem().getIdentifier() + " -> property srs: " + geomPt.getSRS();
                LOG.logDebug( msg );
                if ( createSrsCode == -1 ) {
            ...
            updateGeometryProperty( fid, tablePath, dbField, dbGeometry, targetSrsCode );
    ...
    private void updateSimpleProperty( FeatureId fid, TableRelation[] tablePath, MappingField dbField,
        ...
        if ( tablePath.length == 0 ) {
            updateSimpleProperty( fid, dbField, replacementValue );
    ...
    private void updateGeometryProperty( FeatureId fid, TableRelation[] tablePath, MappingGeometryField dbField,
        ...
        if ( tablePath.length == 0 ) {
            updateGeometryProperty( fid, dbField, replacementValue, targetSrsCode );
        } else if ( tablePath.length == 1 ) {
     ...
     * Updates a simple property of the specified feature.
     * <p>
     * This method handles the case where the property is stored in the feature table itself, so a
     * single UPDATE statement is sufficient.
     * 
     * @param fid
     * @param dbField
     * @param replacementValue
     ...
     * @throws DatastoreException
     */
    private void updateGeometryProperty( FeatureId fid, MappingGeometryField dbField, Object replacementValue, int targetSrsCode )
                            throws DatastoreException {
    ...
    /**
     * Determines the values for the key columns that are referenced by the given table relation (as
     * from fields).
     * 
     * @param fid
     * @param relation
     ...
     * <p>
     * If the row already exists, the existing key is returned, otherwise a new row for the property
     * is inserted first.
     * 
     * @param relation
     * @param dbField
