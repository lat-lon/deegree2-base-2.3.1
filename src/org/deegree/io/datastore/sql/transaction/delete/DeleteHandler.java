//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/datastore/sql/transaction/delete/DeleteHandler.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * Handler for {@link Delete} operations (which usually occur as parts of {@link Transaction}
 * requests).
 * <p>
 ...
 * <li>the {@link FeatureGraph} is built in order to determine which features may be deleted
 * without removing subfeatures of independent features</li>
 * <li>the {@link TableGraph} is built that contains explicit information on all table rows that
 * have to be deleted (and their dependencies)</li>
 * <li>the {@link TableNode}s of the {@link TableGraph} are sorted in topological order, i.e. they
 * may be deleted in that order without violating any foreign key constraints</li>
 * </ul>
 * 
 * @see FeatureGraph
 * @see TableGraph
...
public class DeleteHandler extends AbstractRequestHandler {
     ...
     */
    public DeleteHandler( SQLTransaction dsTa, TableAliasGenerator aliasGenerator, Connection conn,
                          String lockId ) {
    ...
    /**
     * Deletes the features from the {@link Datastore} that have a certain type and are matched by
     * the given filter.
     * 
     * @param ft
    ...
    public int performDelete( MappedFeatureType ft, Filter filter )
        ...
        List<TableNode> sortedNodes = tableGraph.getNodesInTopologicalOrder();
    ...
    /**
     * Adds nodes to the given {@link TableNode} that represent the simple/geometry properties in
    ...
    List<TableNode> determinePropNodes( FeatureId fid, TableRelation relation )
        ...
        try {
            ...
            while ( rs.next() ) {
                ...
                for ( int i = 0; i < toFields.length; i++ ) {
                    KeyColumn column = new KeyColumn( toFields[i].getField(),
                                                      toFields[i].getType(), rs.getObject( i + 1 ) );
     ...
     * @throws DatastoreException
     */
    TableNode determineJTNode( FeatureId fid, FeatureId subFid, TableRelation relation1,
                               TableRelation relation2 )
                            throws DatastoreException {

        LOG.logDebug( "Determining join table entry for feature " + fid + " and subfeature "
                      + subFid );
        ...
        try {
            ...
            if ( rs.next() ) {
                ...
                for ( int i = 0; i < selectFields.length; i++ ) {
                    KeyColumn column = new KeyColumn( selectFields[i].getField(),
                                                      selectFields[i].getType(),
                                                      rs.getObject( i + 1 ) );
                    keyColumns.add( column );
                }
    ...
    /**
     * Returns all stored rows (as {@link TableNode}s) that reference the given row ({@link TableNode})
     * via the also given reference relation.
     * 
     * @param node
     * @param ref
        ...
        try {
            ...
            while ( rs.next() ) {
                ...
                for ( i = 0; i < fromFields.length; i++ ) {
                    KeyColumn column = new KeyColumn( fromFields[i].getField(),
                                                      fromFields[i].getType(), rs.getObject( i + 1 ) );
                    keyColumns.add( column );
                }
    ...
    /**
     * Returns all tables that reference the given table and that are defined in the mapping of the
     * given property type.
     * 
     * @param property
     * @param table
    ...
    private List<TableReference> getReferencingTables( MappedPropertyType property, String table ) {
        ...
        if ( property instanceof MappedFeaturePropertyType
            ...
            for ( MappedFeatureType concreteType : substitutions ) {
                ...
                TableReference ref = new TableReference( getTableRelation( finalStep,
                                                                           concreteType.getTable() ) );
                if ( ref.getToTable().equals( table ) ) {
                    tables.add( ref );
                }
            }
    ...
    private TableRelation getTableRelation( TableRelation toAbstractSubFt, String table ) {
        ...
        for ( int i = 0; i < toConcreteFields.length; i++ ) {
            ...
            toConcreteFields[i] = new MappingField( table, toAbstractField.getField(),
                                                    toAbstractField.getType() );
        }
        TableRelation toConcreteSubFt = new TableRelation( toAbstractSubFt.getFromFields(),
                                                           toConcreteFields,
