//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/datastore/sde/SDEInsertHandler.java $
/*----------------    FILE HEADER  ------------------------------------------
        ...
        if ( relations.length == 1 ) {
            ...
            if ( relations[0].isFromFK() ) {
                ...
                for ( int i = 0; i < pkFields.length; i++ ) {
                    ...
                    if ( pkField == null ) {
                        String msg = "Missing foreign key " + pkField.getColumnName() + " / " + pkField.getTable();
                        throw new TransactionException( msg );
                    }
                    int pkColumnType = pkField.getSQLType();
                    int fkColumnType = fkFields[i].getType();
