//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/io/datastore/PropertyPathResolver.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * Helper class that resolves {@link PropertyPath} instances (e.g. PropertyName elements in
 * {@link GetFeature}) against the property structure of {@link MappedFeatureType}s.
 * 
...
public class PropertyPathResolver {
    ...
    /**
     * Ensures that the requested property begins with a "feature type step" (this may also be an
     * alias).
     * <p>
     * This is necessary, as section 7.4.2 of the Web Feature Implementation Specification 1.1.0
     * states:
     * </p>
     * <p>
    ...
    public static PropertyPath[] normalizePropertyPaths( MappedFeatureType ft, String alias, PropertyPath[] paths ) {
        for ( int i = 0; i < paths.length; i++ ) {
            paths[i] = normalizePropertyPath( ft, alias, paths[i] );
        }
        if ( paths.length == 0 ) {
            ...
            paths = new PropertyPath[] { PropertyPathFactory.createPropertyPath( firstStep ) };
        }
        return paths;
    }
     ...
     * <p>
     * If no properties are specified for a feature type, a single {@link PropertyPath} entry is
     * created that selects the whole feature type.
     * </p>
     * 
     * @param fts
    ...
    /**
     * Determines the properties of the given feature type that have to be fetched based on the
     * requested property paths.
     * <p>
     ...
     * feature, the WFS will internally augment the property name list to include all mandatory
     * property names.
     * </p>
     * <p>
