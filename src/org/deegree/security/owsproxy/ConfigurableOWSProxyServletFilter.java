//$HeadURL: svn+ssh://developername@svn.wald.intevation.org/deegree/base/trunk/src/org/deegree/security/owsproxy/OWSProxyServletFilter.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version 1.1, $Revision: 7461 $, $Date: 2007-06-05 15:35:14 +0200 (Di, 05 Jun 2007) $
 * 
 * @since 1.1
 */
public class ConfigurableOWSProxyServletFilter implements Filter {
    ...
    public void doFilter( ServletRequest request, ServletResponse response, FilterChain chain )
        ...
        if ( !imageExpected ) {
            LOG.logDebug( "Internal response was", new String( b ) );
        } else {
    ...
    /**
     * exports the changed request to a XML document/string that will substitute the original
     * request contained in the passed {@link ServletRequestWrapper} 
     *  
    ...
    /**
     * returns the user from the incomming request.
     * 
     * @param request
    ...
    private User getUser( HttpServletRequest request, OGCWebServiceRequest owsReq )
        ...
        // concrete implementaion not all parameters are used for authentication
        ...
        // the available authentication implementaions and their order are defined in a
        // configuration file. Depending on their order it will be tried to authentificate
        ...
        // So it can be configured which authentication methods in which ord shall be used.
        StringBuffer sb = new StringBuffer( 1000 );
        if ( authList.size() == 0 ) {
            LOG.logInfo( "no authentication method defined, return null as user" );
            return null;
        }
        sb.append( "folloing authentication methods has been performed: " );
        for ( AbstractAuthentication authentication : authList ) {
            try {
                LOG.logDebug( "authenticate using: " + authentication.getAuthenticationName() );
                usr = authentication.authenticate( params );
                if ( usr != null ) {
                    return usr;
                }
                sb.append( "can not get user with authentication method: " );
                sb.append( authentication.getAuthenticationName() ).append( " | " );
            } catch ( WrongCredentialsException e ) {
                LOG.logInfo( "user can not be authenticated with: " + authentication.getAuthenticationName() );
        ...
        // no authentication method succeded (usr is still null)
        String msg = org.deegree.i18n.Messages.getMessage( "OWSPROXY_UNAUTHORIZED_USER", sb );
