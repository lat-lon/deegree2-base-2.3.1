//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/security/owsrequestvalidator/GetCapabilitiesResponseValidator.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version $Revision: 14700 $, $Date: 2008-11-05 14:13:19 +0100 (Mi, 05. Nov 2008) $
 */
    ...
    private Layer removeWMSLayer( Layer layer, User user, SecurityAccess access )
        ...
        for ( int i = 0; i < layers.length; i++ ) {
            if ( layers[i].getName() != null ) {
                ...
                if ( secObj == null || user.getRights( access, secObj ).getRight( secObj, GETMAP ) == null ) {
        ...
        if ( op.isUserCoupled() && user != null ) {
            try {
                ...
                for ( int i = 0; i < ft.length; i++ ) {
                    ...
                    if ( secObj == null || user.getRights( access, secObj ).getRight( secObj, GETFEATURE ) == null ) {
