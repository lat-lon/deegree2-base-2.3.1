//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/security/owsrequestvalidator/GetCapabilitiesResponseValidator.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version $Revision: 10586 $, $Date: 2008-03-14 11:27:57 +0100 (Fri, 14 Mar 2008) $
 */
public class GetCapabilitiesResponseValidator extends ResponseValidator {
     ...
     */
    private byte[] validateWCSCapabilities( @SuppressWarnings("unused")
    Document doc, @SuppressWarnings("unused")
    User user ) {
        // TODO
    ...
    /**
     * recursive method that removes all 'named' layers (layers that has a name in addtion to a
     * title) from the layer tree thats root node (layer) is passed to the method and that not
     * present in the passed <tt>List</tt>
     * 
     * @param layer
     * @param validLayers
     */
    ...
    /**
     * recursive method that removes all 'named' layers (layers that has a name in addition to a
     * title) from the layer tree thats root node (layer) is passed to the method and the passed
     * user doesn't have a GetMap right on.
     * 
     * @param layer
     *            layer to validate
     * @param user
    ...
    private Layer removeWMSLayer( Layer layer, User user, SecurityAccess access )
        ...
        for ( int i = 0; i < layers.length; i++ ) {
            if ( layers[i].getName() != null ) {
                ...
                if ( secObj == null || user.getRights( access, secObj ).getRight( secObj, RightType.GETMAP ) == null ) {
        ...
        if ( op.isUserCoupled() && user != null ) {
            try {
                ...
                for ( int i = 0; i < ft.length; i++ ) {
                    ...
                    if ( secObj == null
                         || user.getRights( access, secObj ).getRight( secObj, RightType.GETFEATURE ) == null ) {
