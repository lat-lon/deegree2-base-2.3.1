//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/security/owsrequestvalidator/wfs/GetFeatureRequestValidator.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version 1.1, $Revision: 10573 $, $Date: 2008-03-13 10:44:08 +0100 (Thu, 13 Mar 2008) $
 * 
 * @since 1.1
 */
class GetFeatureRequestValidator extends AbstractWFSRequestValidator {
    ...
    /**
     * validates if the passed request is valid against the policy assigned to the validator. If the
     * passed user is not <tt>null</tt> user coupled parameters will be validated against a users
    ...
    private void addFilter( GetFeature wfsreq, Condition postConditions, User user )
        ...
        if ( postConditions.getOperationParameter( "instanceFilter" ) != null
            ...
            for ( int i = 0; i < queries.length; i++ ) {
                ...
                if ( LOG.isDebug() ) {
                    LOG.logDebug( "Filter", filter == null ? " is null" : filter.toXML() );
                }
    ...
    /**
     * valides if the format you in a GetFeature request is valid against the policy assigned to
     * Validator. If the passed user is not <tt>null</tt> and the maxFeatures parameter is user
     * coupled the maxFeatures will be validated against a users and rights management system.
     * 
     * @param condition
     * @param maxFeatures
     * @throws InvalidParameterValueException
    ...
    private void validateAgainstRightsDB( GetFeature wfsreq, User user )
        ...
        for ( int i = 0; i < queries.length; i++ ) {
            ...
            handleUserCoupledRules( user, // the user who posted the request
                                    ...
                                    sb.toString(), // the Qualified name of the users
                                    // Featurerequest
