//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/security/owsrequestvalidator/wms/GetMapRequestValidator.java $
/*----------------    FILE HEADER  ------------------------------------------
    ...
    private void validateAgainstRightsDB( GetMap wmsreq, User user )
        ...
        for ( int i = 0; i < layers.length; i++ ) {
            ...
            if ( securityConfig.getProxiedUrl() == null ) {
                ...
                handleUserCoupledRules( user, feature, "[" + securityConfig.getProxiedUrl() + "]:"
                                                       + layers[i].getName(), "Layer", GETMAP );
            }
        }
