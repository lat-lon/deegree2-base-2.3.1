//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/security/owsrequestvalidator/wms/GetMapRequestValidator.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version 1.1, $Revision: 10559 $, $Date: 2008-03-12 09:35:07 +0100 (Wed, 12 Mar 2008) $
 * 
 * @since 1.1
 */
...
public class GetMapRequestValidator extends AbstractWMSRequestValidator {
    ...
    private void validateSLD_Body( Condition condition, StyledLayerDescriptor sld_body ) {
        ...
        /*
         * 
         * OperationParameter op = condition.getOperationParameter( SLD_BODY ); // version is valid
         * because no restrictions are made if ( sld_body == null ||op.isAny() ) return; // at the
         * moment it is just evaluated if the user is allowed // to perform a SLD request or not. no
         * content validation will // be made boolean isAllowed = false; if ( op.isUserCoupled() ) {
         * //TODO // get comparator list from security registry } if (!isAllowed ) { throw new
    ...
    private void validateAgainstRightsDB( GetMap wmsreq, User user )
        ...
        for ( int i = 0; i < layers.length; i++ ) {
            ...
            handleUserCoupledRules( user, feature, layers[i].getName(), "Layer", RightType.GETMAP );
        }
