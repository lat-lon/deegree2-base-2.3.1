// $HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/model/coverage/grid/AbstractGridCoverage.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public abstract class AbstractGridCoverage extends AbstractCoverage implements GridCoverage {
    ...
    protected BufferedImage paintImage( BufferedImage targetImg, float[][] data, Envelope targetEnv,
        ...
        int targetImgHeight = targetImg.getWidth();
    ...
    /**
     * Paint overlapping area from sourceImg into targetImg.
     * This method works with 16bit target pixel and 32bit source pixel.
     * 
     * @param targetImg  the target image.
     * @param sourceImg  the source image.
     * @param xt1  x-coordinate of the first Pixel of sourceImg in targetImg
     * @param yt1  y-coordinate of the first Pixel of sourceImg in targetImg
     * @param xt2  x-coordinate of the last Pixel of sourceImg in targetImg
     * @param yt2  y-coordinate of the last Pixel of sourceImg in targetImg
     */
    private void paintImageT16S32( BufferedImage targetImg,
        BufferedImage sourceImg, int xt1, int yt1, int xt2, int yt2 ) {
      ...
      for ( int i = Math.max( 0, sourceImgWidth -1 - xt2 ); i <= Math.min( targetImgWidth - 1 - xt1, sourceImgWidth -1); i++ ) {
          int targetX = xt1 + i;
          for ( int j = Math.max( 0, sourceImgHeight -1 - yt2 ); j <= Math.min( targetImgHeight -1 - yt1, sourceImgHeight -1 ); j++ ) {
              int targetY = yt1 + j;
    ...
    /**
     * Paint overlapping area from sourceImg into targetImg.
     * This method works with 32bit target pixel and 16bit source pixel.
     * 
     * @param targetImg  the target image.
     * @param sourceImg  the source image.
     * @param xt1  x-coordinate of the first Pixel of sourceImg in targetImg
     ...
     * @param yt2  y-coordinate of the last Pixel of sourceImg in targetImg
     */
    private void paintImageT32S16( BufferedImage targetImg,
        BufferedImage sourceImg, int xt1, int yt1, int xt2, int yt2 ) {
      ...
      // targetX is the running x coordinate of the overlapping area in the target image
      // targetY is the running y coordinate of the overlapping area in the target image
      for ( int i = Math.max( 0, sourceImgWidth - xt2 ); i < Math.min( targetImgWidth - xt1, sourceImgWidth ); i++ ) {
          int targetX = xt1 + i;
          for ( int j = Math.max( 0, sourceImgHeight - yt2 ); j < Math.min( targetImgHeight - yt1, sourceImgHeight ); j++ ) {
              int targetY = yt1 + j;
              int srcPos = sourceImgWidth * j + i;
              int targetPos = targetImgWidth * targetY + targetX;
    ...
    /**
     * Paint overlapping area from sourceImg into targetImg.
     * This method works with 16bit target pixel and 16bit source pixel.
     * 
     * @param targetImg  the target image.
     * @param data
     * @param sourceImg  the source image.
     ...
     * @param yt2  y-coordinate of the last Pixel of sourceImg in targetImg
     */
    private void paintImageT16S16( BufferedImage targetImg, float[][] data,
        BufferedImage sourceImg, int xt1, int yt1, int xt2, int yt2 ) {
      assert sourceImg.getColorModel().getPixelSize() == 16;
      assert targetImg.getColorModel().getPixelSize()  == 16;
    ...
    /**
     * Paint overlapping area from sourceImg into targetImg.
     * This method works with every combination of pixel size in 
     ...
     * @param yt2  y-coordinate of the last Pixel of sourceImg in targetImg
     */
    private void paintImageDefault( BufferedImage targetImg,
        BufferedImage sourceImg, int xt1, int yt1, int xt2, int yt2 ) {
      int xs1 = Math.max(0, sourceImg.getWidth() - 1 - xt2 );
      int xs2 = Math.min( targetImg.getWidth() -1 - xt1, sourceImg.getWidth() - 1 );
      int ys1 = Math.max( 0, sourceImg.getHeight() -1 - yt2 );
      int ys2 = Math.min( targetImg.getHeight() -1 - yt1, sourceImg.getHeight() - 1);
