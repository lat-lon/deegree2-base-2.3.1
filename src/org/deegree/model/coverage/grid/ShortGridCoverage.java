//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/model/coverage/grid/ShortGridCoverage.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * GridCoverage implementation for holding grids stored in a raw byte matrix (byte[][]) or in a set
 * of <tt>ByteGridCoverage</tt>s
 * 
...
public class ShortGridCoverage extends AbstractGridCoverage {
    ...
    public BufferedImage getAsImage( int xAxis, int yAxis ) {
        ...
        if ( data != null ) {
            ...
            for ( int i = 0; i < sources.length; i++ ) {
                ...
                bi = paintImage( bi, data, targetEnv, sourceImg, sourceEnv );
            }
            if ( targetPs == 16 ) {
    ...
    private BufferedImage createBufferedImage( int xAxis, int yAxis ) {
        ...
        case 1: {
            ...
            ComponentColorModel ccm = new ComponentColorModel( cs, null, false, false, BufferedImage.OPAQUE,
                                                               DataBuffer.TYPE_USHORT );
            WritableRaster wr = ccm.createCompatibleWritableRaster( xAxis, yAxis );

            bi = new BufferedImage( ccm, wr, false, new Hashtable() );
        }
        }
        return bi;
    }
