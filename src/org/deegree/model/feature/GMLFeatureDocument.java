//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/model/feature/GMLFeatureDocument.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * schema the structure of the generated features is checked. If no schema information is available,
 * feature + property types are heuristically determined from the feature instance in the document
 * (guessing of simple property types can be turned off, because it may cause unwanted effects).
 * </p>
 * <p>
 * <b>XLinks</b><br>
...
public class GMLFeatureDocument extends GMLDocument {
    ...
    public FeatureProperty parseProperty( Element propertyElement, FeatureType ft, String srsName )
        ...
        if ( propertyType instanceof SimplePropertyType ) {
            ...
            List childElements = XMLTools.getNodes( propertyElement, "*", nsContext );
            ...
            default: {
                String msg = Messages.format( "ERROR_INVALID_FEATURE_PROPERTY2", propertyName, childElements.size() );
                // throw new XMLParsingException( msg );
            }
            }
        }
     ...
     * <p>
     * If schema information is available and a feature type with the given name is not defined, an
    ...
    protected String parseFeatureId( Element featureElement ) {
        ...
        // TODO
        // evaluate the regular expression. The pattern given at
        // http://www.w3.org/TR/xmlschema11-2/ is not a regular expression
        // even if it should be according to http://www.w3.org/TR/xmlschema11-2/#rf-pattern
    ...
    private SimplePropertyType guessSimplePropertyType( String value, QualifiedName propertyName ) {
        ...
        if ( this.guessSimpleTypes ) {
            ...
            /*
             * if (typeCode == Types.VARCHAR) { try { TimeTools.createCalendar( value ); typeCode =
             * Types.DATE; } catch (Exception e) {} }
             */
        }
