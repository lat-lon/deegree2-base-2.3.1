//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/model/spatialschema/GMLGeometryAdapter.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version $Revision: 14125 $, $Date: 2008-09-26 11:14:46 +0200 (Fr, 26. Sep 2008) $
 */
public class GMLGeometryAdapter {
    ...
    public static Envelope wrapBox( Element element, String srsName )
        ...
        if ( nl != null && nl.size() > 0 ) {
            bb = new Position[2];
            bb[0] = createPositionFromCoord( nl.get( 0 ) );
            bb[1] = createPositionFromCoord( nl.get( 1 ) );
        } else {
            nl = XMLTools.getElements( element, COORDINATES, nsContext );
            if ( nl != null && nl.size() > 0 ) {
                bb = createPositionFromCoordinates( nl.get( 0 ) );
            } else {
                nl = XMLTools.getElements( element, POS, nsContext );
                if ( nl != null && nl.size() > 0 ) {
                    bb = new Position[2];
                    bb[0] = createPositionFromPos( nl.get( 0 ) );
                    bb[1] = createPositionFromPos( nl.get( 1 ) );
                } else {
    ...
    private static Position[] createPositionFromPosList( Element element, String srsName )
        ...
        if ( values.length < 4 ) {
            if ( values.length == 2 ) {
                values = new double[] { values[0], values[1], values[0], values[1] };
                size = 2;
            } else {
                for ( double d : values ) {
    ...
    private static Position[] createPositions( Element parent, String srsName )
        ...
        Position[] pos = null;
        if ( nl != null && nl.size() > 0 ) {
            pos = createPositionFromCoordinates( nl.get( 0 ) );
        } else {
            nl = XMLTools.getElements( parent, POS, nsContext );
            if ( nl != null && nl.size() > 0 ) {
                pos = new Position[nl.size()];
                for ( int i = 0; i < pos.length; i++ ) {
                    pos[i] = createPositionFromPos( nl.get( i ) );
                }
            } else {
    ...
    public static PrintWriter export( Geometry geometry, PrintWriter pw )
        ...
        if ( geometry instanceof Point ) {
            exportPoint( (Point) geometry, pw );
        } else if ( geometry instanceof Curve ) {
            exportCurve( (Curve) geometry, pw );
        } else if ( geometry instanceof Surface ) {
            exportSurface( (Surface) geometry, pw );
        } else if ( geometry instanceof MultiPoint ) {
            exportMultiPoint( (MultiPoint) geometry, pw );
        } else if ( geometry instanceof MultiCurve ) {
            exportMultiCurve( (MultiCurve) geometry, pw );
        } else if ( geometry instanceof MultiSurface ) {
            exportMultiSurface( (MultiSurface) geometry, pw );
        } else if ( geometry instanceof MultiGeometry ) {
            exportMultiGeometry( (MultiGeometry) geometry, pw );
        }
        pw.flush();
        return pw;
    }
    ...
    private static void exportPoint( Point point, PrintWriter pw ) {
        ...
        pw.println( srs );
        pw.print( appendPos( point.getPosition(), point.getCoordinateSystem() ) );
        pw.print( "</gml:Point>" );
    ...
    private static void exportCurve( Curve o, PrintWriter pw )
        ...
        for ( int i = 0; i < curveSegments; i++ ) {
            ...
            pw.print( appendPosList( p, o.getCoordinateDimension() ) );
    ...
    private static StringBuilder appendPosList( Position[] p, int coordinateDimension ) {
    ...
    private static StringBuilder appendPos( Position pos, CoordinateSystem crs ) {
        StringBuilder sb = new StringBuilder();
        if ( pos != null ) {
            sb.append( "<gml:pos" );
    ...
    private static void printInteriorRing( Surface surface, PrintWriter pw, SurfacePatch patch ) {
        ...
        if ( ip != null ) {
            for ( int j = 0; j < ip.length; j++ ) {
                ...
                if ( surface.getCoordinateSystem() != null ) {
                    pw.print( appendPosList( ip[j], surface.getCoordinateDimension() ) );
                } else {
                    pw.print( appendPosList( ip[j], 0 ) );
                }
                pw.println( "</gml:LinearRing>" );
                pw.println( "</gml:interior>" );
            }
    ...
    private static void printExteriorRing( Surface surface, PrintWriter pw, SurfacePatch patch ) {
        ...
        if ( surface.getCoordinateSystem() != null ) {
            pw.print( appendPosList( patch.getExteriorRing(), surface.getCoordinateDimension() ) );
        } else {
            pw.print( appendPosList( patch.getExteriorRing(), 0 ) );
        }
        pw.print( "</gml:LinearRing>" );
        pw.print( "</gml:exterior>" );
    }
    ...
    private static void exportMultiPoint( MultiPoint mp, PrintWriter pw ) {
        ...
        for ( int i = 0; i < mp.getSize(); i++ ) {
            ...
            pw.print( appendPos( mp.getPointAt( i ).getPosition(), mp.getCoordinateSystem() ) );
            pw.println( "</gml:Point>" );
        }
        pw.println( "</gml:pointMembers>" );
    ...
    private static void exportMultiCurve( MultiCurve multiCurve, PrintWriter pw )
        ...
        for ( int i = 0; i < curves.length; i++ ) {
            ...
            for ( int j = 0; j < numberCurveSegments; j++ ) {
                ...
                pw.print( appendPosList( p, curve.getCoordinateDimension() ) );
                pw.println( "</gml:LineStringSegment>" );
            }
            pw.println( "</gml:segments>" );
            pw.println( "</gml:Curve>" );
    ...
    public static void exportMultiGeometry( MultiGeometry multiGeometry, PrintWriter pw )
        ...
        // opening tag
        if ( crs != null ) {
            pw.print( "<gml:MultiGeometry srsName=\"" + crs + "\">" );
        } else {
    ...
    /**
     * Converts the string representation of a GML geometry object to a corresponding <code>Geometry</code>. Notice that
     ...
     * @throws GeometryException
     * @throws XMLParsingException
     * @deprecated this method cannot provide default SRS information, please use {@link #wrap(String,String)} instead
     */
    @Deprecated
    ...
    /**
     * Converts a GML geometry object to a corresponding <tt>Geometry</tt>. Notice that GML Boxes will be converted to
     * Surfaces because in ISO 19107 Envelops are no geometries.
     * <p>
     ...
     * @deprecated this method cannot provide default SRS information, please use {@link #wrap(Element,String)} instead
     */
    @Deprecated
     ...
     * @throws InvalidGMLException
     * @throws UnknownCRSException
     * @deprecated this method cannot provide default SRS information, please use {@link #wrapBox(Element,String)}
     *             instead
     */
    @Deprecated
