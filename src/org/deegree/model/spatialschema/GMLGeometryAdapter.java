//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/model/spatialschema/GMLGeometryAdapter.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version $Revision: 11346 $, $Date: 2008-04-22 13:41:53 +0200 (Tue, 22 Apr 2008) $
 */
public class GMLGeometryAdapter {
    ...
    /**
     * Returns true if the given localName != <code>null</code> or empty and matches one of the
     * following:
     * <ul>
     * <li>Point</li>
     * <li>Center</li>
     ...
     * @param localName
     *            to check.
     * @return true if localName equal-ignore-case's one of the above strings.
     */
    ...
    /**
     * Returns an instance of {@link Curve} created from the passed <code>gml:Curve</code>
     * element.
     * 
     * @param element
    ...
    protected static Curve wrapCurveAsCurve( Element element, String srsName )
        ...
        CurveSegment[] segments = parseCurveSegments( crs, segment );
     ...
     * @throws GeometryException
     */
    private static CurveSegment[] parseCurveSegments( CoordinateSystem crs, Element segment )
        ...
        for ( int i = 0; i < list.size(); i++ ) {
            if ( list.get( i ).getLocalName().equals( "LineStringSegment" ) ) {
                segments[i] = parseLineStringSegment( (Element) list.get( i ), crs );
            } else if ( list.get( i ).getLocalName().equals( "Arc" ) ) {
                segments[i] = parseArc( (Element) list.get( i ), crs );
            } else {
                throw new GeometryException( "not supported type for a CurveSegment: " + list.get( i ).getLocalName() );
            }
        }
        return segments;
    }
     ...
     * @throws GeometryException
     */
    private static CurveSegment parseArc( Element element, CoordinateSystem crs )
                            throws GeometryException {
        CurveSegment segment = null;
        try {
            Position[] pos = createPositions( element, null );
            segment = GeometryFactory.createCurveSegment( pos, crs );
        } catch ( Exception e ) {
            throw new GeometryException( "Error creating segments for the element Arc." );
        }
        return segment;
    }
    ...
    protected static MultiCurve wrapMultiCurveAsMultiCurve( Element element, String srsName )
        ...
        try {
            // gml:curveMember
            List listCurveMember = XMLTools.getNodes( element, "gml:curveMember", nsContext );
            ...
            Element curveMembers = (Element) XMLTools.getNode( element, "gml:curveMembers", nsContext );
            if ( curveMembers != null ) {
                // gml:curveMembers
                List listCurves = XMLTools.getNodes( curveMembers, "gml:Curve", nsContext );
                if ( listCurves != null ) {
    ...
    protected static Surface wrapSurfaceAsSurface( Element element, String srsName )
        ...
        for ( int i = 0; i < polygonList.size(); i++ ) {
            ...
            try {
                ...
                surfacePatches[i] = GeometryFactory.createSurfacePatch( exteriorRing, interiorRings, crs );
    ...
    private static String findSrsName( Element element, String srsName )
        ...
        if ( tmp != null ) {
            ...
            tmp = XMLTools.getNodeAsString( elem, "//@srsName", nsContext, srsName );
            if ( tmp != null ) {
                srsName = tmp;
            }
        }
    ...
    protected static MultiSurface wrapMultiSurfaceAsMultiSurface( Element element, String srsName )
        ...
        try {
            ...
            // gml:surfaceMember
            List listSurfaceMember = XMLTools.getNodes( element, "gml:surfaceMember", nsContext );
            ...
            Element surfaceMembers = (Element) XMLTools.getNode( element, "gml:surfaceMembers", nsContext );
            ...
            LOG.logError( e.getMessage(), e );
            String msg = Messages.getMessage( "GEOM_MULTISURFACE_PARSING_ERROR" );
            throw new XMLParsingException( msg );
        } catch ( GeometryException e ) {
            LOG.logError( e.getMessage(), e );
            String msg = Messages.getMessage( "GEOM_MULTISURFACE_FORMAT_ERROR" );
     ...
     * @throws UnknownCRSException
     */
    private static Point wrapPoint( Element element, String srsName )
                            throws XMLParsingException, InvalidGMLException, UnknownCRSException {
    ...
    /**
     * Returns a {@link Curve} instance created from the passed <code>gml:LineString</code>
     * element.
     * 
     * @param element
    ...
    /**
     * Returns a {@link Surface} instance created from the passed <code>gml:Polygon</code>
     * element.
     * 
     * @param element
    ...
    private static Surface wrapPolygon( Element element, String srsName )
        ...
        List nl = XMLTools.getNodes( element, CommonNamespaces.GML_PREFIX + ":outerBoundaryIs", nsContext );
        if ( nl == null || nl.size() == 0 ) {
            nl = XMLTools.getRequiredNodes( element, CommonNamespaces.GML_PREFIX + ":exterior", nsContext );
        }
        Element outs = (Element) nl.get( 0 );
        nl = XMLTools.getRequiredNodes( outs, CommonNamespaces.GML_PREFIX + ":LinearRing", nsContext );
        Element ring = (Element) nl.get( 0 );
        nl = XMLTools.getNodes( ring, COORDINATES, nsContext );
        Position[] outerRing = createPositions( ring, srsName );
        ...
        if ( inns != null && inns.size() > 0 ) {
            ...
            for ( int i = 0; i < innerRings.length; i++ ) {
                ...
                nl = XMLTools.getRequiredNodes( (Node) inns.get( i ), CommonNamespaces.GML_PREFIX + ":LinearRing",
                                                nsContext );

                ring = (Element) nl.get( 0 );
                innerRings[i] = createPositions( ring, srsName );
            }
        }
    ...
    private static MultiPoint wrapMultiPoint( Element element, String srsName )
        ...
        List listPointMember = XMLTools.getNodes( element, "gml:pointMember", nsContext );
        ...
        if ( pointMembers != null ) {
            List pointElems = XMLTools.getNodes( pointMembers, "gml:Point", nsContext );
     ...
     * <code>gml:CompositeSurface</code> element.
     * 
     * TODO
     * 
     * @param element
    ...
    private static Position[] createPositionFromPosList( Element element, String srsName )
        ...
        if ( srsName == null ) {
            if ( srsDimension != null ) {
                throw new InvalidGMLException(
                                               "Attribute srsDimension cannot be defined unless attribute srsName has been defined." );
            }
            if ( axisLabels != null ) {
                throw new InvalidGMLException(
                                               "Attribute axisLabels cannot be defined unless attribute srsName has been defined." );
            }
        ...
        if ( axisLabels == null ) {
            if ( uomLabels != null ) {
                throw new InvalidGMLException(
                                               "Attribute uomLabels cannot be defined unless attribute axisLabels has been defined." );
            }
        }
    ...
    private static void printInteriorRing( Surface surface, PrintWriter pw, SurfacePatch patch ) {
        ...
        if ( ip != null ) {
            for ( int j = 0; j < ip.length; j++ ) {
                ...
                if ( surface.getCoordinateSystem() != null ) {
                    printPositions( pw, ip[j], surface.getCoordinateDimension() );
                } else {
                    printPositions( pw, ip[j], 0 );
                }
                pw.println( "</gml:LinearRing>" );
                pw.println( "</gml:interior>" );
            }
    ...
    private static void printExteriorRing( Surface surface, PrintWriter pw, SurfacePatch patch ) {
        ...
        if ( surface.getCoordinateSystem() != null ) {
            printPositions( pw, patch.getExteriorRing(), surface.getCoordinateDimension() );
        } else {
            printPositions( pw, patch.getExteriorRing(), 0 );
        }
        pw.print( "</gml:LinearRing>" );
        pw.print( "</gml:exterior>" );
    }
     ...
     */
    private static void printRing( PrintWriter pw, Ring ring, int coordinateDimension ) {
        pw.print( "<gml:Ring><gml:curveMember><gml:Curve><gml:segments>" );
        CurveSegment[] cs = ring.getCurveSegments();
        for ( int i = 0; i < cs.length; i++ ) {
            printCurveSegment( pw, cs[i], coordinateDimension );
        }
     ...
     * @param coordinateDimension
     */
    private static void printCurveSegment( PrintWriter pw, CurveSegment segment, int coordinateDimension ) {
        pw.print( "<gml:LineStringSegment>" );
        printPositions( pw, segment.getPositions(), coordinateDimension );
        pw.print( "</gml:LineStringSegment>" );
    }
    ...
    private static void printPositions( PrintWriter pw, Position[] p, int coordinateDimension ) {
        StringBuilder posList = new StringBuilder( "<gml:posList" );
