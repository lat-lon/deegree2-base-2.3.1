//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/model/spatialschema/WKTAdapter.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class WKTAdapter {
    ...
    private static double[] createDoubles( String toSeparate )
        ...
        double[] result = null;
        if ( doubs != null ) {
            result = new double[doubs.length];
            for ( int i = 0; i < doubs.length; ++i ) {
                String d = doubs[i];
                if ( d != null && !"".equals( d ) ) {
                    try {
                        result[i] = Double.valueOf( d );
