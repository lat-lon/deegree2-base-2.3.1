//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/framework/util/BasicUUIDFactory.java $
/*
 * 
...
/**
 * 
 * A hack implementation of something similar to a DCE UUID generator. Generates unique 128-bit
 * numbers based on the current machine's internet address, the current time, and a sequence number.
 * This implementation should be made to conform to the DCE specification. ("DEC/HP, Network
 * Computing Architecture, Remote Procedure Call Runtime Extensions Specification, version OSF
 * TX1.0.11," Steven Miller, July 23, 1992. This is part of the OSF DCE Documentation. Chapter 10
 * describes the UUID generation algorithm.)
 * <P>
 * Some known deficiencies:
 * <ul>
 * <li> Rather than using the 48-bit hardware network address, it uses the 32-bit IP address. IP
 * addresses are not guaranteed to be unique.
 * <li> There is no provision for generating a suitably unique number if no IP address is available.
 * <li> Two processes running on this machine which start their respective UUID services within a
 * millisecond of one another may generate duplicate UUIDS.
 * </ul>
 * <P>
 * However, the intention is that UUIDs generated from this class will be unique with respect to
 * UUIDs generated by other DCE UUID generators.
 * 
 * @deprecated use
 * @see UUID
 * 
