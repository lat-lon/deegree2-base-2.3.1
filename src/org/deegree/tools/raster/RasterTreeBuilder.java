//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/tools/raster/RasterTreeBuilder.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 16Bit and 32Bit tif-images without color model. <br>
 ...
 * <li>overflowToDisk = false (notice that overflow to disk is not supported because cached objects
 ...
 * 
 * @version 2.0, $Revision: 11220 $, $Date: 2008-04-17 10:28:05 +0200 (Thu, 17 Apr 2008) $
 * 
 * @since 2.0
 */
public class RasterTreeBuilder {
    ...
    /**
     * Determins the necessary size of a bounding box, which is large enough to hold all input image
     * files. The result is stored in the combining <code>Envelope</code>.
     * 
     * @throws Exception
     */
    ...
    /**
     * Determins a usefull size for the virtual bounding box. It is somewhat larger than the
     * combining bounding box. The result is stored in the virtual <code>Envelope</code>.
     * 
     */
     ...
     * and vertically (pxHeightTile). It also sets the necessary number of <code>tileCols</code>
     * (depending on the tileWidth) and <code>tileRows</code> (depending on the tileHeight).
     * 
    ...
    private void determineTileSize() {
         ...
         * maxTileSize px, then it has only 1 tile in that direction. In this case, the size of the
         * tile equals the size of the virtual bbox.
         * 
    ...
    private void createTiles( int rows, int cols )
        ...
        double tileWidth = minimumRes * ( pxWidthTile - 1 );
        double tileHeight = minimumRes * ( pxHeightTile - 1 );
    ...
    /**
     * Paints all image files that intersect with the passed <code>tile</code> onto that tile and
     * creates an output file in the <code>outputDir</code>. If no image file intersects with the
     * given tile, then an empty output file is created. The name of the output file is defined by
     * the <code>baseName</code> and the tile's index of row and column.
     * 
     * @param tile
            ...
            if ( imageFilesErrors.get( imageFiles.get( i ) ) == null && file.exists() && !file.isDirectory() ) {
                ...
                if ( wf.getEnvelope().intersects( tileEnv ) ) {
                    ...
                    try {
                        ...
                        drawImage( out, data, bi, tile, wf, minimumRes, interpolation, imageFilesErrors, outputFormat,
                                   bitDepth, offset, scaleFactor );
                        paint = true;
                    } catch ( Exception e ) {
                        e.printStackTrace();
                        imageFilesErrors.put( imageFiles.get( i ), e.getMessage() );
     ...
     * This method is static so it can be used easily from the <code>RasterTreeUpdater</code>.
     * 
     * @param out
     *            target image tile
     * @param data
        ...
        if ( x2 - x1 >= 0 && y2 - y1 >= 0 && x1 + x2 - x1 < image.getWidth() && y1 + y2 - y1 < image.getHeight()
            ...
            if ( !isTransparent( img ) ) {
                ...
                if ( ( wf.getResx() / minimumRes < 0.9999 ) || ( wf.getResx() / minimumRes > 1.0001 )
                    ...
                    newImg = scale( img, interpolation, (float) ( wf.getResx() / minimumRes ),
                                    (float) ( wf.getResy() / minimumRes ) );
                } else {
                    newImg = img;
                }
     ...
     */
    private static BufferedImage scale( BufferedImage img, Interpolation interpolation, float scaleX, float scaleY ) {
    ...
    private static boolean isTransparent( BufferedImage bi ) {
        /*
         * TODO determine if the passed image is completly transparent for ( int i = 0; i <
         * bi.getHeight(); i++ ) { for ( int j = 0; j < bi.getWidth(); j++ ) { if ( bi.getRGB( i, j ) !=
         * 0 && bi.getRGB( i, j ) != -256 ) { return false; } } } return true;
         */
        return false;
    }
     ...
     * @param interpolation
     */
    public static Interpolation createInterpolation( String interpolation ) {
    ...
    /**
     * creates a configuration file (extended CoverageDescriotion) for a WCS coverage considering
     * the passed resolution levels
     * 
    ...
    /**
     * Validates the content of <code>map</code>, to see, if necessary arguments were passed when
     * calling this class.
     * 
     * @param map
     * @throws Exception
    ...
    /**
     * 
     * This class enables sorting of dBaseFile objects in chronological order (lowest first, highest
     * last).
     * 
     * @author <a href="mailto:mays@lat-lon.de">Judit Mays</a>
     * @author <a href="mailto:poth@lat-lon.de">Andreas Poth</a>
