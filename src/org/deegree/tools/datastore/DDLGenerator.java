//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/tools/datastore/DDLGenerator.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * tables and columns used in an annotated GML schema. Some DDL generation may be dependent on the
 * specific SQL backend to be used, so this is implemented in concrete extensions of this class.
 * 
...
public abstract class DDLGenerator {
     ...
     * 
     * @param table
     * @return the DDL statements necessary for the creation of standard indexes for the given table
     *         definition
     */
    protected abstract StringBuffer generateCreateIndexStmts( TableDefinition table );

    /**
     * Generates the DDL statements necessary for the creation of the given sequence. May be
     ...
     * 
     * @param table
     * @return the DDL statements necessary for the dropping of standard indexes for the given table
     *         definition
     */
    ...
    /**
     * Generates the DDL statements necessary for the removal of the given sequence. May be
     * overwritten by a concrete implementation.
     * 
     * @param sequenceName
    ...
    private void buildTableMap( MappedFeatureType featureType ) {
        ...
        if ( gmlId.getIdGenerator() instanceof DBSeqIdGenerator ) {
            extractSequence( (DBSeqIdGenerator) featureType.getGMLId().getIdGenerator() );
        }
    ...
    private void buildTableMap( MappedGeometryPropertyType geometryProperty, TableDefinition table ) {
        ...
        if ( relations == null || relations.length == 0 ) {
            newColumns.add( new ColumnDefinition( geometryProperty.getMappingField().getField(),
                                                  ...
                                                  geometryProperty.getMappingField().getSRS() ) );
        } else {
            TableRelation firstRelation = relations[0];
            MappingField[] fromFields = firstRelation.getFromFields();
            for ( int i = 0; i < fromFields.length; i++ ) {
    ...
    private void buildTableMap( TableRelation[] relations, MappingField targetField ) {
        ...
        if ( targetField instanceof MappingGeometryField ) {
            ...
            column = new ColumnDefinition( targetField.getField(), targetField.getType(), false, true,
                                           ( (MappingGeometryField) targetField ).getSRS() );
        } else {
            column = new ColumnDefinition( targetField.getField(), targetField.getType(), false, false, -1 );
        }
    ...
    /**
     * Generates the DDL statements that can be used to build a relational schema that backs the GML
     * schema.
     * 
     * @param outputFile
     * @throws IOException
        ...
        void addColumn( ColumnDefinition column ) {
            ...
            if ( oldColumn != null ) {
                if ( !( column.getType() == oldColumn.getType() ) ) {
                    String msg = Messages.format( "ERROR_COLUMN_DEFINITION_TYPES", column.getName(),
                                                  oldColumn.isNullable() ? "NULLABLE" : "NOT NULLABLE",
    ...
    class ColumnDefinition {
        ...
        ColumnDefinition( String columnName, int type, boolean isNullable, boolean isPartOfPK,
                          boolean isGeometryColumn, int srsCode ) {
            this( columnName, type, isNullable, isGeometryColumn, srsCode );
