//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wfs/XMLFactory.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class XMLFactory extends org.deegree.owscommon.XMLFactory {
    ...
    public static WFSCapabilitiesDocument export( WFSCapabilities capabilities )
        ...
        try {
            ...
            if ( servesGMLObjectTypes != null ) {
                appendGMLObjectTypeList( root, WFS, PRE_WFS + "ServesGMLObjectTypeList", servesGMLObjectTypes );
            }
            GMLObject[] supportsGMLObjectTypes = capabilities.getSupportsGMLObjectTypeList();
            if ( supportsGMLObjectTypes != null ) {
                appendGMLObjectTypeList( root, WFS, PRE_WFS + "SupportsGMLObjectTypeList", supportsGMLObjectTypes );
            }
    ...
    /**
     * Appends a <code>Keyword</code> -element to the passed <code>Element</code> and fills it
    ...
    private static void appendLock( Element root, Lock lock )
        ...
        Element lockElement = XMLTools.appendElement( root, WFS, PRE_WFS + "Lock" );
        ...
        if ( lock.getFilter() != null ) {
            StringReader sr = new StringReader( lock.getFilter().toXML().toString() );
            Document doc;
            try {
    ...
    public static LockFeatureResponseDocument export( LockFeatureResponse response )
        ...
        Element root = doc.getRootElement();
        XMLTools.appendElement( root, WFS, PRE_WFS + "LockId", response.getLockId() );
        String[] fids = response.getFeaturesLocked();
        if ( fids.length != 0 ) {
            Element featuresLockedElement = XMLTools.appendElement( root, WFS, PRE_WFS + "FeaturesLocked" );
            for ( String fid : fids ) {
        ...
        if ( fids.length != 0 ) {
            Element featuresNotLockedElement = XMLTools.appendElement( root, WFS, PRE_WFS + "FeaturesNotLocked" );
            for ( String fid : fids ) {
    ...
    private static void appendTransactionSummary( Element root, int totalInserted, int totalUpdated, int totalDeleted ) {
        Element taSummary = XMLTools.appendElement( root, WFS, PRE_WFS + "TransactionSummary" );
        XMLTools.appendElement( taSummary, WFS, PRE_WFS + "totalInserted", "" + totalInserted );
        XMLTools.appendElement( taSummary, WFS, PRE_WFS + "totalUpdated", "" + totalUpdated );
        XMLTools.appendElement( taSummary, WFS, PRE_WFS + "totalDeleted", "" + totalDeleted );
    }
    ...
    private static void appendInsertResults( Element root, Collection<InsertResults> insertResults ) {
        if ( insertResults.size() > 0 ) {
            Element insertResultsElement = XMLTools.appendElement( root, WFS, PRE_WFS + "InsertResults" );
    ...
    private static void appendFeatureIds( Element root, InsertResults results ) {
        Element featureElement = XMLTools.appendElement( root, WFS, PRE_WFS + "Feature" );
    ...
    private static void appendQuery( Element root, Query query )
        ...
        Element queryElem = XMLTools.appendElement( root, WFS, PRE_WFS + "Query" );
        ...
        for ( int i = 0; i < propertyNames.length; i++ ) {
            Element propertyNameElement = XMLTools.appendElement( queryElem, WFS, PRE_WFS + "PropertyName" );
    ...
    public static void appendFeatureTypeList( Element root, FeatureTypeList featureTypeList ) {
        ...
        Element featureTypeListNode = XMLTools.appendElement( root, WFS, PRE_WFS + "FeatureTypeList", null );
        Operation[] operations = featureTypeList.getGlobalOperations();
        if ( operations != null ) {
            Element operationsNode = XMLTools.appendElement( featureTypeListNode, WFS, PRE_WFS + "Operations" );
            for ( int i = 0; i < operations.length; i++ ) {
                XMLTools.appendElement( operationsNode, WFS, PRE_WFS + "Operation", operations[i].getOperation() );
            }
        }
    ...
    public static void appendWFSFeatureType( Element root, WFSFeatureType featureType ) {
        ...
        Element featureTypeNode = XMLTools.appendElement( root, WFS, PRE_WFS + "FeatureType" );
        ...
        XMLTools.appendElement( featureTypeNode, WFS, PRE_WFS + "Name", featureType.getName().getPrefixedName() );
        XMLTools.appendElement( featureTypeNode, WFS, PRE_WFS + "Title", featureType.getTitle() );
        String abstract_ = featureType.getAbstract();
        if ( abstract_ != null ) {
            XMLTools.appendElement( featureTypeNode, WFS, PRE_WFS + "Abstract", featureType.getAbstract() );
        }
        ...
        if ( defaultSrs != null ) {
            XMLTools.appendElement( featureTypeNode, WFS, PRE_WFS + "DefaultSRS", defaultSrs.toString() );
            URI[] otherSrs = featureType.getOtherSrs();
            if ( otherSrs != null ) {
                for ( int i = 0; i < otherSrs.length; i++ ) {
                    XMLTools.appendElement( featureTypeNode, WFS, PRE_WFS + "OtherSRS", otherSrs[i].toString() );
                }
            }
        } else {
            XMLTools.appendElement( featureTypeNode, WFS, PRE_WFS + "Title" );
        }
        Operation[] operations = featureType.getOperations();
        if ( operations != null ) {
            Element operationsNode = XMLTools.appendElement( featureTypeNode, WFS, PRE_WFS + "Operations" );
            for ( int i = 0; i < operations.length; i++ ) {
                XMLTools.appendElement( operationsNode, WFS, PRE_WFS + "Operation", operations[i].getOperation() );
            }
        }
    ...
    /**
     * Appends the XML representation of the given {@link GMLObject} (as a
     * <code>wfs:GMLObjectType</code> element) to the passed <code>Element</code>.
     * 
     * @param root
     * @param gmlObjectType
     */
    public static void appendGMLObjectTypeType( Element root, GMLObject gmlObjectType ) {
        ...
        Element gmlObjectTypeNode = XMLTools.appendElement( root, WFS, PRE_WFS + "GMLObjectType" );
        ...
        XMLTools.appendElement( gmlObjectTypeNode, WFS, PRE_WFS + "Name", gmlObjectType.getName().getPrefixedName() );
        if ( gmlObjectType.getTitle() != null ) {
            XMLTools.appendElement( gmlObjectTypeNode, WFS, PRE_WFS + "Title", gmlObjectType.getTitle() );
        }
        String abstract_ = gmlObjectType.getAbstract();
        if ( abstract_ != null ) {
            XMLTools.appendElement( gmlObjectTypeNode, WFS, PRE_WFS + "Abstract", gmlObjectType.getAbstract() );
        }
    ...
    /**
     * Appends the XML representation of the given {@link Envelope} (as an
     * <code>ows:WGS84BoundingBoxType</code> element) to the passed <code>Element</code>.
     * 
     * @param root
     * @param envelope
     */
    ...
    /**
     * Appends the XML representation of the given {@link FormatType}s as (as a
     * <code>wfs:OutputFormats</code> element) to the passed <code>Element</code>.
     * 
     * @param root
     * @param formats
     */
    public static void appendOutputFormats( Element root, FormatType[] formats ) {
        ...
        Element outputFormatsNode = XMLTools.appendElement( root, WFS, PRE_WFS + "OutputFormats" );
        for ( int i = 0; i < formats.length; i++ ) {
            Element formatNode = XMLTools.appendElement( outputFormatsNode, WFS, PRE_WFS + "Format",
                                                         formats[i].getValue() );
