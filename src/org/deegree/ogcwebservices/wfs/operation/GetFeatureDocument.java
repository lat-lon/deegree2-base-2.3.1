//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wfs/operation/GetFeatureDocument.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version $Revision: 14319 $, $Date: 2008-10-16 11:31:18 +0200 (Do, 16. Okt 2008) $
 */
public class GetFeatureDocument extends AbstractWFSRequestDocument {
    ...
    Query parseQuery( Element element, boolean useVersion_1_0_0 )
        ...
        List<Node> nl = null;
        if ( useVersion_1_0_0 ) {
            nl = getNodes( element, "ogc:PropertyName/text()", nsContext );
        } else {
            nl = getNodes( element, "wfs:PropertyName/text()", nsContext );
        }
        PropertyPath[] propertyNames = new PropertyPath[nl.size()];
        for ( int i = 0; i < propertyNames.length; i++ ) {
            propertyNames[i] = parsePropertyPath( (Text) nl.get( i ) );
        }
    ...
    static void isBoundingBoxValid( String srsName, Filter filter )
        ...
        try {
            ...
            bb = new GeoTransformer( create( "EPSG:4326" ) ).transform( bb, bb.getCoordinateSystem() );

            Point2d naturalOrigin = ( (ProjectedCRS) crs.getCRS() ).getProjection().getNaturalOrigin();

            double degx = toDegrees( naturalOrigin.x );

            if ( !( degx > bb.getMin().getY() && degx < bb.getMax().getY() ) ) {
                throw new InvalidParameterValueException( get( "WFS_WRONG_UTM_STRIPE", bb.getMin().getY(), degx,
                                                               bb.getMax().getY() ) );
            }
