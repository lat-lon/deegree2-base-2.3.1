//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wmps/PrintMapHandler.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version 2.0, $Revision: 9546 $, $Date: 2008-01-15 17:11:15 +0100 (Tue, 15 Jan 2008) $
 */
public class PrintMapHandler implements Runnable {
     ...
     */
    private BufferedImage performBuildMapImage( Map config_layers, PrintMap printMap, Envelope bbox, double width,
                                                double height )
                            throws OGCWebServiceException {
     ...
     */
    private void handleLayerDatasources( Map config_layers, PrintMap printMap, Envelope bbox, double width,
                                         double height, Graphics g )
                            throws OGCWebServiceException, InconsistentRequestException {

        double px = WMPSConfiguration.INCH2M
                    / configuration.getDeegreeParams().getPrintMapParam().getTargetResolution();
     ...
     * @return Layer[]
     */
    private GetMap.Layer[] getContinuousVectorLayer( Map config_layers, GetMap.Layer[] printMapLayers, double mapScale,
                                                     int i ) {
    ...
    /**
     * Parses the Layer datastores to determine the type of the layer. Layers having a vector
     * datasource as well as a raster datasource for the passed mapScale will be treated as raster
     * layers
     * 
     * @param layer
     * @param mapScale
     ...
     * @return Envelope
     */
    private Envelope createBBOX( Point center, int scaleDenominator, double mapWidth, double mapHeight ) {
        if ( LOG.getLevel() == ILogger.LOG_DEBUG ) {
            ...
            LOG.logDebug( "target resolution (dpi): ",
                          configuration.getDeegreeParams().getPrintMapParam().getTargetResolution() );
    ...
    private int[] parseImageNodes( Element root )
        ...
        try {
            ...
            List<Node> images = XMLTools.getRequiredNodes( root, "detail/band/image", null );
            for ( int i = 0; i < images.size(); i++ ) {
                ...
                if ( idx != -1 ) {
                    ...
                    Element reportElement = (Element) XMLTools.getRequiredNode( image, "reportElement", null );
