//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/csw/manager/CatalogueHarvester.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version 1.0. $Revision: 10580 $, $Date: 2008-03-13 11:46:01 +0100 (Thu, 13 Mar 2008) $
 * 
 * @since 2.0
 */
public class CatalogueHarvester extends AbstractHarvester {
    ...
    protected class HarvestProcessor extends AbstractHarvestProcessor {
        ...
        public void run() {
            ...
            try {
                ...
                for ( int i = 0; i < typeNames.length; i++ ) {
                    int index = 1;
                    int hits = getNoOfMetadataRecord( source, typeNames[i] );
                    LOG.logInfo( hits + " metadatasets to harvest ..." );
                    for ( int j = 0; j < hits; j++ ) {

                        try {
                            // read index'th metadata set from CSW
                            metaData = getNextMetadataRecord( source, index, typeNames[i] );
                            if ( metaData != null ) {
                                ...
                                try {
                                    ...
                                    // remove fileIdentifier of current dataset from list of inserted
                                ...
                                LOG.logInfo( "harvesting will be stopped at index: " + index
                                             + " because metadata == null" );
                            }
                            LOG.logDebug( index + " metadata " + ( metaData == null ) );
                        } catch ( Throwable e ) {
                            LOG.logError( Messages.format( "CatalogueHarvester.exception3", index, "not available",
                                                           source ), e );
                            try {
                                e.printStackTrace();
                                // inform handlers assigend to the harvest request about failure
                ...
                // CSW. Even if harvesting a few records has failed s harvest process will declared
        ...
        private void deleteRecordsNoHostedAnymore( URI source )
            ...
            for ( int i = 0; i < cache.size(); i++ ) {
                ...
                if ( record == null ) {
                    ...
                    String trans = createDeleteRequest( fid, "./iso19115:fileIdentifier/smXML:CharacterString" );
                    performTransaction( trans );
                }
            }
        }
        ...
        private XMLFragment getNextMetadataRecord( URI source, int index, String type )
            ...
            // TODO
            // read different templates depending on metadata format
            URL url = CatalogueHarvester.class.getResource( "iso09_getrecords_template.xml" );
            ...
            Node node = XMLTools.getNode( xml.getRootElement(), "csw:SearchResults/child::*[1]", nsc );
            if ( node != null ) {
        ...
        private int getNoOfMetadataRecord( URI source, String type )
            ...
            // TODO
            // read different templates depending on metadata format
            URL url = CatalogueHarvester.class.getResource( "iso09_get_no_of_records_template.xml" );
            String getRecords = FileUtils.readTextFile( url ).toString();
            getRecords = StringTools.replace( getRecords, "$type$", type, false );
            ...
            return XMLTools.getNodeAsInt( xml.getRootElement(), "csw:SearchResults/@numberOfRecordsMatched", nsc, 0 );
