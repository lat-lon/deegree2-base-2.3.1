//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/csw/manager/TransactionDocument.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class TransactionDocument extends AbstractCSWRequestDocument {
    ...
    public Transaction parse( String id )
        ...
        boolean verbose = XMLTools.getNodeAsBoolean( getRootElement(), "./@verboseResponse", nsContext,
                                                     false );
    ...
    private Update parseUpdate( Element element )
        ...
        List children = null;
        List rp = XMLTools.getNodes( getRootElement(), "./csw:RecordProperty", nsContext );
        if ( rp.size() != 0 ) {
            ...
            if ( children.size() == 0 ) {
                throw new XMLParsingException(
                                               "one record must be defined within a CS-W update element" );
            }
        }
