//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/csw/discovery/GetRecords.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * @version $Revision: 9970 $, $Date: 2008-02-08 09:38:12 +0100 (Fri, 08 Feb 2008) $
 */
...
public class GetRecords extends AbstractCSWRequest {
    ...
    public static GetRecords create( Map<String, String> kvp )
        ...
        String outputSchema = getParam( "OUTPUTSCHEMA", kvp, DEFAULT_OUTPUTSCHEMA );
        ...
        SortProperty[] sortProperties = SortProperty.create( kvp.remove( "SORTBY" ) );
     ...
     * 
     * @return the mapped namespaces or <code>null</code> if all qualified names are in default
     *         namespace.
     * 
     */
    ...
    /**
     * sets the resultType of a request. This may be useful to perform a request first with
     * resultType = HITS to determine the total number of records matching a query and afterwards
     * performing the same request with resultType = RESULTS (and maxRecords &lt; number of matched
     * records).
     * 
     * @param resultType
     */
     ...
     * 
     * @return <= 0 if no distributed search shall be performed. otherwise the recursion depht is
     *         returned.
     * 
     */
    ...
    /**
     * @return the number of the first returned dataset. Zero or one (Optional)Default value is 1.
     *         If startPosition > the number of datasets satisfying the constraint, no dataset will
     *         be returned
     * 
     */
    ...
    /**
     * @return The maxRecords parameter. It is used to define the maximum number of records that
     *         should be returned from the result set of a query. If it is not specified, then 10
     *         records shall be returned. If its value is set to zero, then the behavior is
     *         indentical to setting "resultType=HITS"
     * 
     */
    ...
    public static enum RESULT_TYPE {
        /**
         * HITS, the catalogue service shall return an empty &lt;GetRecordsResponse&gt;element with
         * the numberOfRecordsMatched attribute set to indicate the number of hits
         */
        HITS,
        /**
        ...
        VALIDATE,
        /**
         * RESULTS, the catalogue service should generate a complete response with the
