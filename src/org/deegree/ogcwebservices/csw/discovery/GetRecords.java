//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/csw/discovery/GetRecords.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * search.
 * <p>
 * Parameters specific to the <code>GetRecords</code> -request (omitting REQUEST, SERVICE and VERSION): <table
 * border="1">
 * <tr>
 * <th>Name</th>
 * <th>Occurences</th>
 ...
 * schemas must be advertised in the capabilities document.</tr>
 * <tr>
 ...
 * <td align="center">0|1</td>
 * <td>The ResponseHandler parameter is a flag that indicates how the GetRecords operation should be processed by a
 * CSW. If the parameter is not present, then the GetRecords operation is processed synchronously meaning that the
 * client sends the GetRecords request to a CSW and waits to receive a valid response or exception message. The CSW
 * immediately processes the GetRecords request while the client waits for a response. The problem with this mode of
 * operation is that the client may timeout waiting for the CSW to process the request. If the ResponseHandler parameter
 * is present, the GetRecords operation is processed asynchronously. In this case, the CSW responds immediately to a
 * client's request with an acknowledgment message that tells the client that the request has been received and
 * validated, and notification of completion will be sent to the URI specified as the value of the ResponseHandler
 * parameter.</td>
 * </tr>
 * </table>
 * 
 ...
 * 
 * @version $Revision: 13843 $, $Date: 2008-09-08 09:43:10 +0200 (Mo, 08. Sep 2008) $
 */
