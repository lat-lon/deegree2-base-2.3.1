//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/csw/discovery/Query.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class Query {
    ...
    /**
     * @return an Array of element names returned by a getRecord request.
     * @deprecated this method is replaced with the more correct
     *             {@link #getElementNamesAsPropertyPaths()}. For now it just returns the values of
     *             {@link PropertyPath#getAsString()} method or <code>null</code> if no
     *             elementName were requested.
     *             
     */
    @Deprecated
    public String[] getElementsNames() {
        if ( elementNamesAsPropertyPaths == null ) {
            return null;
        }
        String[] r = new String[elementNamesAsPropertyPaths.size()];
        for ( int i = 0; i < elementNamesAsPropertyPaths.size(); ++i ) {
            r[i] = elementNamesAsPropertyPaths.get( i ).getAsString();
        }
        return r;
    }
