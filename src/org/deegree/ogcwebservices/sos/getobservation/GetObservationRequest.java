//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/sos/getobservation/GetObservationRequest.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class GetObservationRequest extends AbstractOGCWebServiceRequest {
    ...
    public static GetObservationRequest create( String id, Document doc )
        ...
        try {
            ...
            List platformIdList = XMLTools.getNodes( doc, "sos:GetObservation/sos:platformID", nsContext );
            ArrayList<String> platformId = new ArrayList<String>( platformIdList.size() );
            for ( int i = 0; i < platformIdList.size(); i++ ) {
                platformId.add( XMLTools.getRequiredNodeAsString( (Node) platformIdList.get( i ), "text()", nsContext ) );
            }
            ...
            for ( int i = 0; i < sensorIdList.size(); i++ ) {
                sensorId.add( XMLTools.getRequiredNodeAsString( (Node) sensorIdList.get( i ), "text()", nsContext ) );
            }
     ...
     * @throws OGCWebServiceException
     */
    private static ArrayList<Object> getTimeList( List timeList )
        ...
        for ( int i = 0; i < timeList.size(); i++ ) {
            ...
            if ( ( XMLTools.getNode( (Node) timeList.get( i ), "gml:TPeriod", nsContext ) ) != null ) {
                ...
                String temp = XMLTools.getNodeAsString( (Node) timeList.get( i ), "gml:TInstant/gml:tPosition/text()",
                                                        nsContext, null );
                if ( temp == null ) {
