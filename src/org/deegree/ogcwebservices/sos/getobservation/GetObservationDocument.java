//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/sos/getobservation/GetObservationDocument.java $
...
public class GetObservationDocument extends OGCDocument {
    ...
    private ObservationArray getObservationsFromDocuments( Document doc, Envelope bbox, String sensorId )
        ...
        List observations = XMLTools.getNodes( doc, "om:ObservationArray/om:observationMembers/gml:Observation",
                                               nsContext );
    ...
    private String[] getPlatformIdsFromAttachedToInSensorDocs( Document[] docs, SOSDeegreeParams deegreeParams )
        ...
        for ( int a = 0; a < docs.length; a++ ) {
            ...
            for ( int i = 0; i < nl.size(); i++ ) {
                String platformIdPropertyValue = XMLTools.getNodeAsString( (Node) nl.get( i ),
    ...
    private String[] getPlatformIdsFromPlatformDocs( Document[] docs, SOSDeegreeParams deegreeParams )
        ...
        try {
            ...
            for ( int a = 0; a < docs.length; a++ ) {
                ...
                List itemsList = XMLTools.getNodes( docs[a], "sml:Platforms/sml:Platform", nsContext );
                for ( int i = 0; i < itemsList.size(); i++ ) {
                    String platformIdPropertyValue = XMLTools.getRequiredNodeAsString( (Node) itemsList.get( i ),
                                                                                       "@id", nsContext );
    ...
    private String[] getSensorIdsFromCarriesInPlatformDoc( Document[] docs, SOSDeegreeParams deegreeParams )
        ...
        try {
            ...
            for ( int x = 0; x < docs.length; x++ ) {
                ...
                List platformNodeList = XMLTools.getNodes( docs[x], "sml:Platforms/sml:Platform", nsContext );

                for ( int i = 0; i < platformNodeList.size(); i++ ) {
                    List carriesNodeList = XMLTools.getNodes( (Node) platformNodeList.get( i ), "sml:carries",
                                                              nsContext );

                    for ( int a = 0; a < carriesNodeList.size(); a++ ) {
                        String sensorIdPropertyValue = XMLTools.getRequiredNodeAsString(
                                                                                         (Node) carriesNodeList.get( a ),
     ...
     * @return the modified operation; modify the scs query to create the correct wfs querry
     ...
     * @param compOp
     * @param measuresConfig
     * @return the modified ComparisonOperation
     * @throws FilterConstructionException
     */
