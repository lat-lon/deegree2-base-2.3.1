//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/sos/WFSRequestGenerator.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class WFSRequestGenerator {
    ...
    private static void setQueryTypeName( Document doc, QualifiedName typename )
        ...
        Element query = (Element) XMLTools.getRequiredNode( doc, "wfs:GetFeature/wfs:Query", nsContext );
        query.setAttribute( "xmlns:" + typename.getPrefix(), typename.getNamespace().toASCIIString() );
     ...
     * @throws XMLParsingException
     */
    public static Document createIsLikeOperationWFSRequest( String[] literals, QualifiedName featureType,
        ...
        for ( int i = 0; i < literals.length; i++ ) {
            ...
            al.add( new PropertyIsLikeOperation( new PropertyName( propertyName ), new Literal( literals[i] ), '*',
                                                 '#', '!' ) );
        }

        // wenn nur ein feature abgefragt wird, dass <or> weglassen
        if ( al.size() == 1 ) {
            Filter filter = new ComplexFilter( al.get( 0 ) );
    ...
    public static Document createBBoxWFSRequest( Envelope bbox, QualifiedName featureTypeName,
                            ...
                            throws IOException, SAXException, XMLParsingException, GeometryException {
        ...
        SpatialOperation bboxOperation = new SpatialOperation( OperationDefines.BBOX,
                                                               new PropertyName( coordPropertyName ), geom );
     ...
     * @throws XMLParsingException
     */
    public static Document createObservationWFSRequest( Object[] times, QualifiedName featureTypeName,
                                                        QualifiedName timePropertyName, Operation filterOperation )
                            throws IOException, SAXException, XMLParsingException {
        ...
        ArrayList<Operation> timeOperationList = new ArrayList<Operation>();

        // creates the time Filters
        for ( int i = 0; i < times.length; i++ ) {
            // if TInstant
            if ( times[i] instanceof TInstant ) {
                Operation op = new PropertyIsCOMPOperation( OperationDefines.PROPERTYISEQUALTO,
                                                            new PropertyName( timePropertyName ),
                                                            new Literal( ( (TInstant) times[i] ).getTPosition() ) );
                timeOperationList.add( op );
                // if TPeriod
            } else if ( times[i] instanceof TPeriod ) {

                ArrayList<Operation> al = new ArrayList<Operation>();
                Operation op = new PropertyIsCOMPOperation( OperationDefines.PROPERTYISGREATERTHANOREQUALTO,
                                                            new PropertyName( timePropertyName ),
                                                            new Literal( ( (TPeriod) times[i] ).getBegin() ) );
                al.add( op );
        ...
        if ( timeOperationList.size() == 1 ) {
            timeOp = timeOperationList.get( 0 );
        ...
        if ( ( timeOp != null ) && ( filterOperation != null ) ) {
            ...
            Filter filter = new ComplexFilter( new LogicalOperation( OperationDefines.AND, operationList ) );
            WFSRequestGenerator.setFilter( request, filter );
        } else if ( timeOp != null ) {
