//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/sos/SOService.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class SOService implements OGCWebService {
    ...
    /**
     * privater constructor to construct a new SCService instance with the configuration as the only
     * parameter
     * 
     * @param scsConfiguration
     * 
     ...
     * 
     * @return the ServiceTypeId from the ServiceIdentification section of the configuration
     */
     ...
     * 
     * @return the Version from the configuration
     */
     ...
     * 
     * @param request
     * @return a DescribePlatformResult
    ...
    private DescribePlatformResult describePlatform( DescribePlatformRequest request )
        ...
        for ( int i = 0; i < types.length; i++ ) {
            if ( serviceConfiguration.getSOSDeegreeParams().getPlatformConfiguration( types[i] ) == null ) {
                throw new OGCWebServiceException( "InvalidParameterValueException: Platform '" + types[i]
                                                  + "' not found" );
            }
        }
    ...
    private DescribeSensorResult describeSensor( DescribeSensorRequest request )
        ...
        for ( int i = 0; i < types.length; i++ ) {
            if ( serviceConfiguration.getSOSDeegreeParams().getSensorConfiguration( types[i] ) == null ) {
                throw new OGCWebServiceException( "InvalidParameterValueException: Sensor '" + types[i] + "' not found" );
            }
        }

        SensorDescriptionDocument sdd = new SensorDescriptionDocument();
        SensorMetadata[] sensors = sdd.getSensor( serviceConfiguration.getSOSDeegreeParams(), request.getTypeNames() );
    ...
    private GetObservationResult getObservation( GetObservationRequest request )
        ...
        for ( int i = 0; i < platforms.length; i++ ) {
            if ( dp.getPlatformConfiguration( platforms[i] ) == null ) {
                throw new OGCWebServiceException( "InvalidParameterValueException: Platform '" + platforms[i]
                                                  + "' not found" );
            }
        }
        ...
        for ( int i = 0; i < sensors.length; i++ ) {
            if ( dp.getSensorConfiguration( sensors[i] ) == null ) {
                throw new OGCWebServiceException( "InvalidParameterValueException: Sensor '" + sensors[i]
                                                  + "' not found" );
            }
        }

        GetObservationDocument god = new GetObservationDocument();
        ObservationArray[] obAr = god.getObservations( serviceConfiguration.getSOSDeegreeParams(), request );
