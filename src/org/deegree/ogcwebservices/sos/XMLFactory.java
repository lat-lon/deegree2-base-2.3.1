//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/sos/XMLFactory.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class XMLFactory extends org.deegree.owscommon.XMLFactory {
    ...
    public static XMLFragment export( SOSCapabilities scsCapabilities )
        ...
        try {
            ...
            ServiceIdentification serviceIdentification = scsCapabilities.getServiceIdentification();
            if ( serviceIdentification != null ) {
                appendServiceIdentification( root, serviceIdentification );
            }

            // appends the ServiceProvider
            ServiceProvider serviceProvider = scsCapabilities.getServiceProvider();
            if ( serviceProvider != null ) {
                appendServiceProvider( root, serviceProvider );
            }

            // appends the OpertionsMetadata
            OperationsMetadata operationsMetadata = scsCapabilities.getOperationsMetadata();
            if ( operationsMetadata != null ) {
                appendOperationsMetadata( root, operationsMetadata );
            }

            // appends the PlatformList
            ArrayList<Platform> platformList = scsCapabilities.getPlatformList();
            if ( platformList != null ) {
                appendPlatformList( root, platformList );
            }

            // appends the SensorList
            ArrayList<Sensor> sensorList = scsCapabilities.getSensorList();
            if ( sensorList != null ) {
    ...
    public static XMLFragment export( DescribePlatformResult describePlatformResult )
        ...
        try {
            ...
            PlatformMetadata[] platformMetadata = describePlatformResult.getPlatforms();
    ...
    public static XMLFragment export( GetObservationResult getObservationResult )
        ...
        try {
            ...
            Element observationMembers = doc.createElement( "om:observationMembers" );
            root.appendChild( observationMembers );

            // add observation
            appendObservationArrays( getObservationResult.getObservations(), observationMembers, doc );
     ...
     * @param doc
     */
    private static void appendObservationArrays( ObservationArray[] observationArrays, Element element, Document doc ) {
        ...
        for ( int i = 0; i < observationArrays.length; i++ ) {
            ...
            minX.appendChild( doc.createTextNode( "" + observationArrays[i].getBoundedBy().getMin().getX() ) );

            Element minY = doc.createElement( "gml:Y" );
            coordMin.appendChild( minY );

            minY.appendChild( doc.createTextNode( "" + observationArrays[i].getBoundedBy().getMin().getY() ) );
            ...
            maxX.appendChild( doc.createTextNode( "" + observationArrays[i].getBoundedBy().getMax().getX() ) );

            Element maxY = doc.createElement( "gml:Y" );
            coordMax.appendChild( maxY );

            maxY.appendChild( doc.createTextNode( "" + observationArrays[i].getBoundedBy().getMax().getY() ) );
            ...
            appendObservations( observationArrays[i].getObservations(), observationMembers, doc );
     ...
     * @param doc
     */
    private static void appendObservations( Observation[] observations, Element element, Document doc ) {
     ...
     * 
     */
    private static void appendPlatformList( Element root, ArrayList<Platform> pl ) {
        ...
        for ( int i = 0; i < pl.size(); i++ ) {
            ...
            elem.setAttribute( "Id", pl.get( i ).getId() );
            ...
            elem.setAttribute( "Description", pl.get( i ).getDescription() );
        }
    }
     ...
     * 
     */
    private static void appendSensorList( Element root, ArrayList<Sensor> sl ) {
        if ( sl == null ) {
            return;
        }
        ...
        for ( int i = 0; i < sl.size(); i++ ) {
            ...
            elem.setAttribute( "Id", sl.get( i ).getId() );
            elem.setAttribute( "Description", sl.get( i ).getDescription() );
     ...
     * @param actPlatform
     * @param doc
     * @return a platform element
     */
    private static Element appendPlatform( PlatformMetadata actPlatform, Document doc ) {
     ...
     * @param doc
     */
    private static void appendIdentifiedAs( Identifier[] actidentifier, Element element, Document doc ) {
        ...
        for ( int i = 0; i < actidentifier.length; i++ ) {
            ...
            identifier.appendChild( doc.createTextNode( actidentifier[i].getIdentifierValue() ) );
            ...
            if ( actidentifier[i].getIdentifierCodeSpace() != null )
                identifier.setAttribute( "codeSpace", actidentifier[i].getIdentifierCodeSpace() );
    ...
    private static void appendAttachedTo( Object object, Element element, Document doc ) {
        ...
        if ( ( (ComponentMetadata) object ).getAttachedTo() != null ) {
            ...
            component.appendChild( doc.createTextNode( ( (ComponentMetadata) object ).getAttachedTo() ) );
            element.appendChild( attachedTo );
        }
    }

    private static void appendClassifiedAs( Classifier[] actClassifier, Element element, Document doc ) {
        ...
        for ( int i = 0; i < actClassifier.length; i++ ) {
            ...
            // required; no check
            classifier.appendChild( doc.createTextNode( actClassifier[i].getValue() ) );
            // optional; check
            if ( actClassifier[i].getCodeSpace() != null ) {
                classifier.setAttribute( "codeSpace", actClassifier[i].getCodeSpace() );
            }
    ...
    private static void appendDerivedFrom( ResponseModel[] responseModels, Element element, Document doc ) {
        ...
        for ( int i = 0; i < responseModels.length; i++ ) {
            ...
            appendIdentifiedAs( responseModels[i].getIdentifiedAs(), responseModel, doc );

            // appends classifiedAs
            appendClassifiedAs( responseModels[i].getClassifiedAs(), responseModel, doc );

            // appends description
            appendDescription( responseModels[i].getDescription(), responseModel, doc );

            appendUsesParametersFromDerivedFrom( responseModels[i].getUsesParameters(), responseModel, doc );

        }

    }

    private static void appendHasCRS( EngineeringCRS engineeringCRS, Element element, Document doc ) {
    ...
    private static void appendEngineeringCRS( EngineeringCRS engineeringCRS, Element element, Document doc ) {
    ...
    private static void appendCoordinateReferenceSystem( CoordinateReferenceSystem crs, Element element, Document doc ) {
    ...
    private static void appendDescribedBy( Object object, Element element, Document doc ) {

        if ( ( (ComponentMetadata) object ).getDescribedBy() != null ) {
            Element describedBy = doc.createElement( "sml:describedBy" );

            Element componentDescription = doc.createElement( "sml:ComponentDescription" );
            describedBy.appendChild( componentDescription );

            if ( ( ( (ComponentMetadata) object ).getDescribedBy() ).getId() != null ) {
                componentDescription.setAttribute( "id", ( ( (ComponentMetadata) object ).getDescribedBy() ).getId() );
            }
            ...
            // append operatedBy
            CitedResponsibleParty[] operatedBy = ( ( (ComponentMetadata) object ).getDescribedBy() ).getOperatedBy();
            for ( int i = 0; i < operatedBy.length; i++ ) {
                appendOperatedBy( operatedBy[i], componentDescription, doc );
            }

            // append operatedBy
            CitedResponsibleParty[] manufactedBy = ( ( (ComponentMetadata) object ).getDescribedBy() ).getManufacturedBy();
            for ( int i = 0; i < manufactedBy.length; i++ ) {
                appendManufactedBy( manufactedBy[i], componentDescription, doc );
            }

            // append operatedBy
            CitedResponsibleParty[] deployedBy = ( ( (ComponentMetadata) object ).getDescribedBy() ).getDeployedBy();
            ...
            if ( ( reference != null ) && ( reference.length > 0 ) ) {
                ...
                for ( int i = 0; i < reference.length; i++ ) {
                    if ( reference[i].isOnLineResource() ) {
                        ...
                        appendOnlineResource( reference[i].getOnLineResource(), ref, doc );
                    }
    ...
    private static void appendLocatedUsing( LocationModel[] locatedUsingList, Element element, Document doc ) {
        ...
        for ( int i = 0; i < locatedUsingList.length; i++ ) {
            ...
            if ( locatedUsingList[i] instanceof GeoPositionModel ) {
                Element geoPositionModel = doc.createElement( "sml:GeoPositionModel" );
                locatedUsing.appendChild( geoPositionModel );

                if ( ( (GeoPositionModel) locatedUsingList[i] ).getId() != null ) {
                    geoPositionModel.setAttribute( "id", ( (GeoPositionModel) locatedUsingList[i] ).getId() );
                }

                appendIdentifiedAs( ( (GeoPositionModel) locatedUsingList[i] ).getIdentifiedAs(), geoPositionModel, doc );

                appendClassifiedAs( ( (GeoPositionModel) locatedUsingList[i] ).getClassifiedAs(), geoPositionModel, doc );

                appendDescription( ( (GeoPositionModel) locatedUsingList[i] ).getDescription(), geoPositionModel, doc );

                // append sourceCRS
                Element sourceCRS = doc.createElement( "sml:sourceCRS" );
                geoPositionModel.appendChild( sourceCRS );
                appendEngineeringCRS( ( (GeoPositionModel) locatedUsingList[i] ).getSourceCRS(), sourceCRS, doc );

                // append referenceCRS
                Element referenceCRS = doc.createElement( "sml:referenceCRS" );
                geoPositionModel.appendChild( referenceCRS );
                appendCoordinateReferenceSystem( ( (GeoPositionModel) locatedUsingList[i] ).getReferenceCRS(),
                                                 referenceCRS, doc );

                // append usesParameters
                appendUsesParametersFromGeoLocation(
                                                     ( (GeoPositionModel) locatedUsingList[i] ).getUsesParametersObjects(),
                                                     geoPositionModel, doc );
    ...
    private static void appendUsesParametersFromGeoLocation( Object[] objects, Element element, Document doc ) {
        ...
        for ( int i = 0; i < objects.length; i++ ) {
            ...
            if ( objects[i] instanceof GeoLocation ) {
                Element usesParameters = doc.createElement( "sml:usesParameters" );
                ...
                geoLocation.setAttribute( "id", ( (GeoLocation) objects[i] ).getId() );

                // appends latitude
                Element latitude = doc.createElement( "sml:latitude" );
                geoLocation.appendChild( latitude );
                appendQuantity( ( (GeoLocation) objects[i] ).getLatitude(), latitude, doc );
                ...
                if ( ( (GeoLocation) objects[i] ).getAltitude() != null ) {
                    ...
                    appendQuantity( ( (GeoLocation) objects[i] ).getAltitude(), altitude, doc );
                }
                ...
                if ( ( (GeoLocation) objects[i] ).getTrueHeading() != null ) {
                    ...
                    appendQuantity( ( (GeoLocation) objects[i] ).getTrueHeading(), trueHeading, doc );
                }
                ...
                if ( ( (GeoLocation) objects[i] ).getSpeed() != null ) {
                    ...
                    appendQuantity( ( (GeoLocation) objects[i] ).getSpeed(), speed, doc );
                }
    ...
    private static void appendUsesParametersFromDerivedFrom( Object[] objects, Element element, Document doc ) {
        if ( objects != null ) {
            for ( int i = 0; i < objects.length; i++ ) {
                // is GeoLocation
                if ( objects[i] instanceof BasicResponse ) {
                    ...
                    Element basicResponse = doc.createElement( "sml:BasicResponse" );
                    ...
                    Element typedQuantity = doc.createElement( "sml:TypedQuantity" );
                    resolution.appendChild( typedQuantity );

                    typedQuantity.appendChild( doc.createTextNode( ""
                                                                   + ( (BasicResponse) objects[i] ).getResolution().getValue() ) );

                    // sets required attrib type
                    typedQuantity.setAttribute( "type",
                                                ( (BasicResponse) objects[i] ).getResolution().getType().toString() );

                    // sets optional attrib codeSpace
                    if ( ( (BasicResponse) objects[i] ).getResolution().getCodeSpace() != null ) {
                        typedQuantity.setAttribute(
                                                    "codeSpace",
                                                    ( (BasicResponse) objects[i] ).getResolution().getCodeSpace().toString() );
                    }
                    ...
                    if ( ( (BasicResponse) objects[i] ).getResolution().getUom() != null ) {
                        typedQuantity.setAttribute( "uom",
                                                    ( (BasicResponse) objects[i] ).getResolution().getUom().toString() );
                    }

                    // sets optional attrib min
                    if ( !Double.isNaN( ( (BasicResponse) objects[i] ).getResolution().getMin() ) ) {
                        typedQuantity.setAttribute( "min", "" + ( (BasicResponse) objects[i] ).getResolution().getMin() );
                    }

                    // sets optional attrib max
                    if ( !Double.isNaN( ( (BasicResponse) objects[i] ).getResolution().getMax() ) ) {
                        typedQuantity.setAttribute( "max", "" + ( (BasicResponse) objects[i] ).getResolution().getMax() );
                    }

                    // sets optional attrib id
                    if ( ( (BasicResponse) objects[i] ).getResolution().getId() != null ) {
                        typedQuantity.setAttribute( "id", ( (BasicResponse) objects[i] ).getResolution().getId() );
                    }
    ...
    private static void appendQuantity( Quantity quantity, Element element, Document doc ) {
     ...
     * @param doc
     */
    private static void appendDescription( Discussion[] description, Element element, Document doc ) {
     ...
     * @param doc
     */
    private static void appendOnlineResource( OnlineResource onlineResource, Element element, Document doc ) {
    ...
    private static void appendOperatedBy( CitedResponsibleParty responsibleParty, Element element, Document doc ) {
    ...
    private static void appendManufactedBy( CitedResponsibleParty responsibleParty, Element element, Document doc ) {
    ...
    private static void appendDeployedBy( CitedResponsibleParty responsibleParty, Element element, Document doc ) {
    ...
    private static void appendContactInfo( ContactInfo contact, Element element, Document doc ) {
        ...
        if ( contact.getContactInstructions() != null ) {
            ...
            instr.appendChild( doc.createTextNode( contact.getContactInstructions() ) );
        }
    ...
    private static void appendAddress( Address address, Element element, Document doc ) {
        ...
        if ( address.getAdministrativeArea() != null ) {
            Element administrativeArea = doc.createElement( "iso19115:administrativeArea" );
            addr.appendChild( administrativeArea );
            administrativeArea.appendChild( doc.createTextNode( address.getAdministrativeArea() ) );
        }
        ...
        for ( int i = 0; i < electronicMailAddresses.length; i++ ) {
            Element electronicMailAddress = doc.createElement( "iso19115:electronicMailAddress" );
            addr.appendChild( electronicMailAddress );
            electronicMailAddress.appendChild( doc.createTextNode( electronicMailAddresses[i] ) );
        }

    }

    private static void appendResponsibleParty( CitedResponsibleParty responsibleParty, Element element, Document doc ) {
        ...
        if ( responsibleParty.getIndividualName() != null ) {
            ...
            for ( int i = 0; i < individualNameList.length; i++ ) {
                ...
                individualName.appendChild( doc.createTextNode( individualNameList[i] ) );
        ...
        if ( responsibleParty.getOrganisationName() != null ) {
            String[] organisationNameList = responsibleParty.getOrganisationName();
            for ( int i = 0; i < organisationNameList.length; i++ ) {
                Element organisationName = doc.createElement( "iso19115:organisationName" );
                organisationName.appendChild( doc.createTextNode( organisationNameList[i] ) );
        ...
        if ( responsibleParty.getPositionName() != null ) {
            ...
            for ( int i = 0; i < positionNameList.length; i++ ) {
                Element positionName = doc.createElement( "iso19115:positionName" );
                positionName.appendChild( doc.createTextNode( positionNameList[i] ) );
        ...
        for ( int i = 0; i < roleList.length; i++ ) {
            if ( roleList[i].getValue() != null ) {
                ...
                Element codeList = doc.createElement( "iso19115:CI_RoleCode_CodeList" );
                role.appendChild( codeList );
                codeList.appendChild( doc.createTextNode( roleList[i].getValue() ) );
            }
        }
    ...
    private static void appendMeasures( Product[] products, Element element, Document doc ) {
        ...
        for ( int i = 0; i < products.length; i++ ) {
            ...
            if ( products[i].getObservable() != null ) {
                ...
                if ( products[i].getObservable().getId() != null ) {
                    phenomenon.setAttribute( "id", products[i].getObservable().getId() );
                }

                if ( products[i].getObservable().getName() != null ) {
                    Element name = doc.createElement( "sml:name" );
                    phenomenon.appendChild( name );
                    name.appendChild( doc.createTextNode( products[i].getObservable().getName() ) );
                }
                ...
                if ( products[i].getObservable().getDefinition() != null ) {
                    ...
                    description.appendChild( doc.createTextNode( products[i].getObservable().getDefinition() ) );
                }
            }
    ...
    private static void appendCarries( String[] asset, Element element, Document doc ) {
        if ( asset == null ) {
            return;
        }
