//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/sos/capabilities/CapabilitiesDocument.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class CapabilitiesDocument extends OWSCommonCapabilitiesDocument {
    ...
    public OGCCapabilities parseCapabilities()
        ...
        try {
            return new SOSCapabilities( parseVersion(), parseUpdateSequence(), getServiceIdentification(),
                                        getServiceProvider(), getOperationsMetadata(), null, getPlatformList(),
                                        getSensorList() );
    ...
    public OperationsMetadata getOperationsMetadata()
        ...
        Operation describePlatform = getOperation( SOSOperationsMetadata.DESCRIBE_PLATFORM_NAME, true, operations );
        // 'DescribeSensor'-operation
        Operation describeSensor = getOperation( SOSOperationsMetadata.DESCRIBE_SENSOR_NAME, false, operations );
        // 'GetObservation'-operation
        Operation getObservation = getOperation( SOSOperationsMetadata.GET_OBSERVATION_NAME, true, operations );

        return new SOSOperationsMetadata( getCapabilites, describePlatform, describeSensor, getObservation );
    }
    ...
    protected ArrayList<Platform> getPlatformList()
        ...
        if ( platformListElement != null ) {
            ...
            if ( ( platformElements != null ) && ( platformElements.getLength() > 0 ) ) {

                for ( int i = 0; i < platformElements.getLength(); i++ ) {
                    String platformId = XMLTools.getRequiredAttrValue( "Id", null, platformElements.item( i ) );
    ...
    protected ArrayList<Sensor> getSensorList()
        ...
        if ( sensorListElement != null ) {
            ...
            if ( ( sensorElements != null ) && ( sensorElements.getLength() > 0 ) ) {

                for ( int i = 0; i < sensorElements.getLength(); i++ ) {
                    String sensorId = XMLTools.getRequiredAttrValue( "Id", null, sensorElements.item( i ) );
