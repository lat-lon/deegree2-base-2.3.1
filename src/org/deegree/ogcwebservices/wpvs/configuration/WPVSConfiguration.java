//$$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wpvs/configuration/WPVSConfiguration.java $$
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * $Revision: 10698 $, $Date: 2008-03-26 14:02:33 +0100 (Wed, 26 Mar 2008) $
 * 
 */
public class WPVSConfiguration extends WPVSCapabilities {
    ...
    public WPVSConfiguration( String version, String updateSequence, ServiceIdentification serviceIdentification,
        ...
        if ( Double.isInfinite( smallestMinimalScaleDenomiator ) || smallestMinimalScaleDenomiator < 0 || smallestMinimalScaleDenomiator > 1) {
                    ...
                    if ( largestElevModel != null ) {
                        ...
                        if ( definedCRSs != null && definedCRSs.length > 0 ) {
                            ...
                            if ( configuredBBox.getWidth() >= configuredBBox.getHeight() ) {
                                requestHeight = (int) Math.floor( ( configuredBBox.getHeight() / configuredBBox.getWidth() ) * MAX_REQUEST_SIZE );
                            } else {
                                requestWidth = (int) Math.floor( ( configuredBBox.getWidth() / configuredBBox.getHeight() ) * MAX_REQUEST_SIZE );
                            }
                            ...
                            if ( largestElevModel instanceof LocalWCSDataSource ) {
                                heightMap = invokeWCS( (LocalWCSDataSource) largestElevModel,
                                                       configuredBBox,
                                                       requestWidth,
                                                       requestHeight );
                            } else if ( largestElevModel instanceof LocalWFSDataSource ) {
                                heightMap = invokeWFS( (LocalWFSDataSource) largestElevModel,
                                                       configuredBBox,
                                                       requestWidth,
                                                       requestHeight );
                            }
                            ...
                            if ( heightMap.length == 0 ) {
                                ...
                                if ( LOG.getLevel() == ILogger.LOG_DEBUG ) {
                                    BufferedImage img = new BufferedImage( requestWidth,
                                                                           requestHeight,
                                    ...
                                    try {
                                        ...
                                        LOG.logDebug( "creating tmpfile for global heightmap with name: " + f.toString() );
                                        f.deleteOnExit();
    ...
    private float[][] invokeWFS( LocalWFSDataSource dataSource, Envelope env, int requestWidth, int requestHeight ) {
        ...
        if ( service == null ) {
            throw new RuntimeException( "No Web Feature Service instance available for creation of the Global height map." );
        }
        ...
        if ( response != null && response instanceof FeatureResult ) {
            ...
            if ( fc != null ) {
                ...
                float[][] result = new float[requestHeight][requestWidth];
                for ( float[] t : result ) {
                    Arrays.fill( t, min );
                }
            ...
            LOG.logError( "ERROR creating a global heightmap while invoking wfs-datasource : " + dataSource.getName()
    ...
    public static double getHeightForPosition( Point3d pos ) {
        ...
        if ( posY < 0 || posY > heightMap.length || posX < 0 || posX > heightMap[0].length ) {
            LOG.logDebug( "Given position " + pos
                          + " is outside the global height lookup, returning minimal value: "
                          + globalMinimalHeight );
            return globalMinimalHeight;
        }
        LOG.logDebug( "The looked up value for postion: " + pos
                      + " (mapped to: "
                      + posX
                      + ", "
                      + posY
    ...
    private float[][] invokeWCS( LocalWCSDataSource dataSource, Envelope env, int requestWidth, int requestHeight ) {
        ...
        if ( service == null ) {
            throw new RuntimeException( "No Web Coverage Service instance available for creation of the Global height map." );
        }
        ...
        try {
            ...
            if ( !Thread.currentThread().isInterrupted() ) {
                LOG.logError( Messages.getMessage( "WPVS_WCS_REQUEST_ERROR",
                                                   "WCSException",
                                                   dataSource.getName(),
                                                   wcse.getMessage() ) );
            }
        ...
        if ( coverageResponse != null && coverageResponse instanceof ResultCoverage ) {
            ...
            if ( response.getCoverage() != null && response.getCoverage() instanceof ImageGridCoverage ) {
                ...
                if ( LOG.getLevel() == ILogger.LOG_DEBUG ) {
                    try {
                        ...
                        LOG.logDebug( "creating tmpfile for global wcs elevationmodel response with name: " + f.toString() );
                        f.deleteOnExit();
            ...
            LOG.logWarning( Messages.getMessage( "WPVS_IVALID_WCS_RESPONSE",
                                                 dataSource.getName(),
    ...
    /**
     * @return an elevation datasource with the largest min scale denominator or <code>null</code> if no elevation
     *         model datasource was found.
     */
    ...
    private AbstractDataSource findLargestElevModel( Dataset dataset ) {
        ...
        if ( model != null ) {
            ...
            if ( sources != null ) {
                for ( AbstractDataSource source : sources ) {
                    if ( source != null && ( elevSource == null || source.getMinScaleDenominator() > elevSource.getMinScaleDenominator() ) ) {
                        elevSource = source;
                    }
                }
            }
        ...
        for ( Dataset set : children ) {
            ...
            if ( tmpSource != null && ( elevSource == null || tmpSource.getMinScaleDenominator() > elevSource.getMinScaleDenominator() ) ) {
