//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wpvs/DefaultGetViewHandler.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * 
 * $Revision: 10703 $, $Date: 2008-03-26 14:06:07 +0100 (Wed, 26 Mar 2008) $
 * 
 */
public class DefaultGetViewHandler extends GetViewHandler {
    ...
    public GetViewResponse handleRequest( final GetView request )
        ...
        if ( resultingBoxes != null && resultingBoxes.size() > 0 ) {
            ...
            renderer = new OffScreenWPVSRenderer( canvas, config.getDeegreeParams().getNearClippingPlane(), theScene,
                                                  request.getImageDimension().width, request.getImageDimension().height );
            ...
            /**
             * If using the TestWPVS class option NO_NEW_REQUEST, make sure to comment following
             * line as well as the universe.cleanup(); and view.removeAllCanvas3Ds(); lines in the
     ...
     * value, i.e. the configured MinimalTerrainHeight.
     * 
     * @param eyePositionX
    ...
    private WPVSScene createScene( GetView request, ViewPoint viewPoint, List<ResolutionStripe> stripes,
        ...
        if ( dgmType == ResolutionStripe.ELEVATION_MODEL_POINTS ) {
            ...
            /*
             * First find the resolution stripe with the best (==highest) resolution, and while
             * doing so, add all measurepoints to the point array.
             */
            ...
            if ( "BBOX".equals( splittingMode ) ) {
                ...
                /**
                 * The goal is to create one large textures to which all other textures are upscaled
                 * and then painted upon. To do so, first get the largest texture, which scale will
                 * be used to create an image (the resulttexture) which has the the dimensions of
                 * the scenes bbox. All that has to be done then, is to calculate for each the
                 * stripe, the relative position of it's bbox, to the scene's bbox and the scale of
                 * it's texture compared to the result texture's dimensions. This only has one
                 * little drawback, if the scene is large (a steep few for example) the texture can
                 * get larger as the available texture width of the gpu, in this case another
                 * scaling has to be applied.
                 */
                ...
                /**
                 * We tried to set the rendering hints to different values here, but it seems, the
                 * standard values result in the quickest and best quality, because j3d will do
                 * antialiasing itself again.
                 */
                ...
                for ( ResolutionStripe stripe : stripes ) {
                    ...
                    if ( stripeTexture != null ) {
                        ...
                        /**
                         * First calculate the position of this stripe's texture as if it was to be
                         * drawn upon it's own all fitting texture.
                         */
                        ...
                        /**
                         * It might happen, that the viratula all-fitting-texture of the upscaled
                         * texture would have been to large for the texture memory to fit, therefore
                         * this scaling factor has to be taken into account as well.
                         */
                         ...
                         * =(tmpTextureWidth*maxTextureScale) and height=
                         * (tmpTextureHeight*maxTextureScale) ], to the real result texture.
                         */
                        ...
                        /**
                         * First translate then set the scale of the g2d and after the drawing reset
                         * the transformation.
                         */
     ...
     * @return a scale to fit the width and height in the configured maximumtexture width. or 1 if
     *         both are smaller.
     */
