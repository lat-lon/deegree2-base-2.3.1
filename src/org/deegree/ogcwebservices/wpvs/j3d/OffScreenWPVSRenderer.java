//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wpvs/j3d/OffScreenWPVSRenderer.java $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 * represent by a <code>BufferedImage</code>. That is, the returned BufferedImage of this class's renderScene method
...
public class OffScreenWPVSRenderer extends Abstract3DRenderingEngine {
     ...
     *            of the resulting image
     */
    public OffScreenWPVSRenderer( Canvas3D canvas, WPVSScene scene, int width, int height ) {
        this( canvas, 2, scene, width, height );
    }
    ...
    public OffScreenWPVSRenderer( Canvas3D canvas, double nearClippingPlane, WPVSScene scene, int width, int height ) {
        ...
        if ( offScreenCanvas3D == null ) {
            LOG.logError( "Did not retrieve a Canvas3D from the Configuraion, cannot proceed" );
            return;
        }
