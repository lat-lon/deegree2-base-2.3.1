//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wpvs/utils/ResolutionStripe.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * The <code>ResolutionStripe</code> class encapsulates a Surface with a maximum Resolution, which is convenient for
 ...
 * 
 * @version $Revision: 10700 $, $Date: 2008-03-26 14:04:29 +0100 (Wed, 26 Mar 2008) $
 * 
 */
...
public class ResolutionStripe implements Callable<ResolutionStripe>, Comparable<ResolutionStripe> {
    ...
    public ResolutionStripe( Surface surface, double maximumResolution, double minimumResolution, double minimalHeight,
        ...
        if ( rH > WPVSConfiguration.MAX_REQUEST_SIZE || rH < 0 || rW > WPVSConfiguration.MAX_REQUEST_SIZE || rW < 0 ) {
            minResolution = ( ( rH > rW ) ? surface.getEnvelope().getHeight() : surface.getEnvelope().getWidth() ) / WPVSConfiguration.MAX_REQUEST_SIZE;
            maxResolution = minResolution;
            debugString = "fixed resolution: " + minimumResolution;
            LOG.logDebug( "Setting maxResolution to " + minResolution
                          + " instead of "
                          + minimumResolution
    ...
    public void setElevationModelDataSource( AbstractDataSource elevationModelDataSource ) {
        ...
        if ( elevationModelDataSource.getServiceType() == AbstractDataSource.LOCAL_WFS || elevationModelDataSource.getServiceType() == AbstractDataSource.REMOTE_WFS ) {
            dgmType = ResolutionStripe.ELEVATION_MODEL_POINTS;
        } else if ( elevationModelDataSource.getServiceType() == AbstractDataSource.LOCAL_WCS || elevationModelDataSource.getServiceType() == AbstractDataSource.REMOTE_WCS ) {
        ...
        if ( featureCollectionDataSources.size() == 0 ) {
            ...
            for ( int i = 0; i < featureCollectionDataSources.size(); ++i ) {
                sb.append( " - " )
                  .append( i )
                  .append( ") " )
                  .append( featureCollectionDataSources.get( i ) )
                  .append( "\n" );
            }
        }
    ...
    public void outputTextures() {
        ...
        for ( String key : keys ) {
            try {
                ...
                f.deleteOnExit();
                LOG.logDebug( "Saving result texture ( in file: " + f.getAbsolutePath()
                              + " for resolution stripe\n"
                              + this );
    ...
    private void invokeDataSource( AbstractDataSource ads, int id ) {
        try {
            ...
            if ( ads.getServiceType() == AbstractDataSource.LOCAL_WMS || ads.getServiceType() == AbstractDataSource.REMOTE_WMS ) {
                invoker = new WMSInvoker( this, id );
            } else if ( ads.getServiceType() == AbstractDataSource.LOCAL_WCS || ads.getServiceType() == AbstractDataSource.REMOTE_WCS ) {
...
//                J3DToCityGMLExporter exporter = new J3DToCityGMLExporter( "test", surface.getCoordinateSystem().getName(), "bla", f.getParent()+File.separator+"textures", 0, 0, 0, false );
...
//        }
    ...
    private void createTexture() {
        ...
        if ( resultTexture == null ) {
            ...
            resultTexture = new BufferedImage( getRequestWidthForBBox(),
            ...
            if ( texturesDataSources.size() > 0 ) {
                ...
                for ( OGCWebServiceException ogcwse : exceptions ) {
                    ...
                    String message = StringTools.concat( 100,
                                                         "error (",
    ...
    private void paintString( Graphics2D g2d, String[] stringsToPaint ) {
        ...
        int stringOffset = (int) Math.round( ( requestHeight * ( 1d / ( stringsToPaint.length + 1 ) ) ) + ( height * 0.5 ) );
