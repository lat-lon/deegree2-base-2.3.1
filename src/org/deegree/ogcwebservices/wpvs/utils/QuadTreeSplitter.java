//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wpvs/utils/QuadTreeSplitter.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class QuadTreeSplitter {
            ...
            if ( LOG.getLevel() == ILogger.LOG_DEBUG ) {
                if ( nrOfStripes < quadMergeCount ) {
                    LOG.logDebug( "Not merging leaves because there are less leaves: " + nrOfStripes
                                  + " then configured quadMergeCount:"
                                  + quadMergeCount );
                }
                try {
        ...
        for ( Double resolution : keys ) {
            if ( lhm.containsKey( resolution ) ) {
                ...
                for ( QuadNode quad : result ) {
                    ...
                    Position newMin = GeometryFactory.createPosition( envMin.getX() - extraWidth,
                                                                      envMin.getY() - extraHeight,
                                                                      envMin.getZ() );

                    Position newMax = GeometryFactory.createPosition( envMax.getX() + extraWidth,
                                                                      envMax.getY() + extraHeight,
                                                                      envMax.getZ() );
                    ...
                    try {
                        Surface resultSurface = GeometryFactory.createSurface( GeometryFactory.createEnvelope( newMin,
                                                                                                               newMax,
                                                                                                               this.crs ),
                                                                               this.crs );
                        ResolutionStripe rs = new ResolutionStripe( resultSurface,
                                                                    maxResolution,
                                                                    minResolution,
                    ...
                    if ( first.canMerge( second ) ) {
                        try {
                            ...
                            if ( rH < WPVSConfiguration.MAX_REQUEST_SIZE && rW < WPVSConfiguration.MAX_REQUEST_SIZE ) {
                                ...
                                LOG.logDebug( "Cannot merge two quads, because their merged sizes( rw: " + rW
                                              + ", rH: "
                                              + rH
                                              + " would be bigger as the maximum retrievable size:\n"
                                              + WKTAdapter.export( GeometryFactory.createSurface( requestEnvelope, crs ) )
                                                          .toString()
                                              + "\n"
                                              + WKTAdapter.export( second.getBBox() ).toString() );
    ...
    private void createTree( QuadNode father, double quadResolution )
        ...
        if ( sonsResolution < minimalResolution ) {
            LOG.logDebug( "The currentresolution (" + sonsResolution
                          + ") is smaller then the minimalResolution("
                          + minimalResolution
                          + ")" );
            return;
        }
        ...
        checkSon( father,
                  sonsResolution,
                  QuadNode.LOWER_LEFT_SON,
                  lowerLeftX,
                  lowerLeftY,
                  lowerLeftX + widthHalf,
                  lowerLeftY + heightHalf );

        // lowerright
        checkSon( father,
                  sonsResolution,
                  QuadNode.LOWER_RIGHT_SON,
                  lowerLeftX + widthHalf,
                  lowerLeftY,
                  lowerLeftX + ( 2 * widthHalf ),
                  lowerLeftY + heightHalf );

        // upperleft
        checkSon( father,
                  sonsResolution,
                  QuadNode.UPPER_LEFT_SON,
                  lowerLeftX,
                  lowerLeftY + heightHalf,
                  lowerLeftX + widthHalf,
    ...
    private void checkSon( QuadNode father, double quadResolution, final int SON_ID, double lowerLeftX,
        ...
        if ( intersectedStripe != null ) { // found an intersecting resolutionStripe
            ...
            QuadNode son = new QuadNode( bbox,
            ...
            if ( ( rH > WPVSConfiguration.MAX_REQUEST_SIZE || rW > WPVSConfiguration.MAX_REQUEST_SIZE ) || quadResolution >= ( intersectResolution * 0.95 ) ) {
    ...
    private class QuadNode implements Comparable<QuadNode> {
        ...
        void addSon( final int sonID, QuadNode son ) {
            if ( sonID == LOWER_LEFT_SON || sonID == LOWER_RIGHT_SON
                 || sonID == UPPER_LEFT_SON
        ...
        boolean canMerge( QuadNode other ) {
            ...
            if ( Math.abs( comparePosition - otherPosition ) < 0.01 ) {
                ...
                if ( Math.abs( compareLength - otherLength ) < 0.01 ) {
                    ...
                    return ( Math.abs( thisMaxX - otherMinX ) < 0.00001 ) || ( Math.abs( thisMinX - otherMaxX ) < 0.00001 );
