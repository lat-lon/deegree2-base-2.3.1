//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wms/DefaultGetMapHandler.java $
/*----------------    FILE HEADER  ------------------------------------------
...
public class DefaultGetMapHandler implements GetMapHandler {
    ...
    public OGCWebServiceResponse performGetMap()
        ...
        for ( int i = 0; i < layers.length; i++ ) {
            ...
            if ( layers[i] instanceof NamedLayer ) {
                ...
                invokeNamedLayer( layers[i], styleName, themes );
            } else {
                double sc = scale;
                if ( !version130 ) {
                    // required because for WMS 1.3.0 'scale' represents the ScaleDenominator
    ...
    /**
     * creates a StyledLayerDocument containing all requested layer, nested layers if required and
     * assigend styles. Not considered are nested layers for mixed requests (LAYERS- and SLD(_BODY)-
    ...
    private StyledLayerDescriptor toSLD( GetMap.Layer[] layers, StyledLayerDescriptor inSLD )
        ...
        if ( layers != null && layers.length > 0 && inSLD == null ) {
            ...
            // add nested layers for mixed case, nested from original sld
            AbstractLayer[] as = inSLD.getLayers();
            for ( AbstractLayer l : as ) {
                addNestedLayers( l, sld );
            }
        } else {
     ...
     */
    private GetMapResult renderMap( List<ExecutionFinishedEvent<Object>> results ) {
