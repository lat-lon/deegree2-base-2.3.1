//$HeadURL: svn+ssh://aerben@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wms/XMLFactory_1_3_0.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * <code>XMLFactory_1_3_0</code> is an XML factory that outputs valid WMS 1.3.0 documents. It is
 * not intended for direct use but is used automatically by the standard <code>XMLFactory</code>.
 * 
 * @author <a href="mailto:schmitz@lat-lon.de">Andreas Schmitz</a>
 * @author last edited by: $Author: aschmitz $
 * 
 * @version $Revision: 14787 $, $Date: 2008-11-07 11:42:28 +0100 (Fr, 07. Nov 2008) $
 */
...
public class XMLFactory_1_3_0 extends XMLFactory {
    ...
    public static WMSCapabilitiesDocument_1_3_0 export( WMSCapabilities_1_3_0 capabilities )
        ...
        try {
            ...
            for ( String f : capabilities.getExceptions() ) {
                XMLTools.appendElement( exc, WMSNS, PWMS + "Format", f );
            }
    ...
    protected static void appendService( Element root, ServiceIdentification identification, ServiceProvider provider )
        ...
        if ( identification.getAccessConstraints().size() > 0 ) {
            XMLTools.appendElement( root, WMSNS, PWMS + "AccessConstraints",
    ...
    protected static void appendContactInformation( Element root, ServiceProvider provider ) {
        ...
        if ( provider.getServiceContact().getContactInfo().length > 0 ) {
            ...
            if ( addr.getAdministrativeArea() != null ) {
                XMLTools.appendElement( caNode, WMSNS, PWMS + "StateOrProvince", addr.getAdministrativeArea() );
            }
            ...
            if ( phone.getVoice().length > 0 ) {
                XMLTools.appendElement( ciNode, WMSNS, PWMS + "ContactVoiceTelephone", phone.getVoice()[0] );
            }
            if ( phone.getFacsimile().length > 0 ) {
                XMLTools.appendElement( ciNode, WMSNS, PWMS + "ContactFacsimileTelephone", phone.getFacsimile()[0] );
            }
    ...
    protected static void appendMetadataURL( Element root, MetadataURL metadataURL ) {
        ...
        Element res = XMLTools.appendElement( elem, WMSNS, PWMS + "OnlineResource" );
        org.deegree.model.metadata.iso19115.XMLFactory.appendOnlineResource( res, metadataURL.getOnlineResource() );
    }
    ...
    protected static void appendLayerBoundingBox( Element root, LayerBoundingBox lBox ) {
        ...
        if ( lBox.getSRS().startsWith( "EPSG" ) ) {
    ...
    protected static void appendLatLonBoundingBox( Element root, Envelope llBox ) {
        ...
        XMLTools.appendElement( bbNode, WMSNS, PWMS + "westBoundLongitude", "" + llBox.getMin().getX() );
        XMLTools.appendElement( bbNode, WMSNS, PWMS + "eastBoundLongitude", "" + llBox.getMax().getX() );
        XMLTools.appendElement( bbNode, WMSNS, PWMS + "southBoundLatitude", "" + llBox.getMin().getY() );
        XMLTools.appendElement( bbNode, WMSNS, PWMS + "northBoundLatitude", "" + llBox.getMax().getY() );
    }
