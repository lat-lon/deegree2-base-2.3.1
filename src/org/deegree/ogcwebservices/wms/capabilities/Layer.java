//$HeadURL: svn+ssh://jwilden@scm.wald.intevation.org/deegree/base/trunk/src/org/deegree/ogcwebservices/wms/capabilities/Layer.java $
/*----------------    FILE HEADER  ------------------------------------------
...
/**
 * Each available map is advertised by a &lt;Layer&gt; element in the Capabilities XML. A single
 * parent Layer encloses any number of additional layers, which may be hierarchically nested as
 * desired. Some properties defined in a parent layer are inherited by the children it encloses.
 ...
 * desired, layers may be repeated in different categories when relevant. No controlled vocabulary
 * has been defined, so at present Layer and Style Names, Titles and Keywords are arbitrary.
 * </p>
...
public class Layer {
    ...
    /**
     * If, and only if, a layer has a &lt;Name&gt;, then it is a map layer that can be requested by
     * using that Name in the LAYERS parameter of a GetMap request. If the layer has a Title but no
     * Name, then that layer is only a category title for all the layers nested within. A Map Server
     * that advertises a Layer containing a Name element shall be able to accept that Name as the
     * value of LAYERS argument in a GetMap request and return the corresponding map. A Client shall
     * not attempt to request a layer that has a Title but no Name.
     * 
     * @return the name
     */
    ...
    /**
     * A &lt;Title&gt; is required for all layers; it is a human-readable string for presentation in
     * a menu. The Title is not inherited by child Layers.
     * 
     * @return the title
     */
    ...
    /**
     * Abstract is a narrative description of the map layer. The Abstract elements are not inherited
     * by child Layers.
     * 
     * @return the abstract
     */
    ...
    /**
     * KeywordList contains zero or more Keywords to aid in catalog searches. The KeywordList
     ...
     * least one &gt;SRS&gt; element that is either stated explicitly or inherited from a parent
     * Layer . The root &lt;Layer&gt; element shall include a sequence of zero or more SRS elements
     * listing all SRSes that are common to all subsidiary layers. Use a single SRS element with
     * empty content (like so: "&lt;SRS&gt;&lt;/SRS&gt; ") if there is no common SRS. Layers may
     * optionally add to the global SRS list, or to the list inherited from a parent layer. Any
     * duplication shall be ignored by clients. When a Layer is available in several Spatial
     * Reference Systems, there are two ways to encode the list of SRS values. The first of these is
     * new in this version of the specification, the second is deprecated but still included for
     * backwards compatibility.
     * <p>
     * 1. Optional, recommended: Multiple single-valued &lt;SRS&gt; elements: a list of SRS values
     * is represented as a sequence of &lt;SRS&gt; elements, each of which contains only a single
     * SRS name. Example: &lt;SRS&gt;EPSG:1234&lt;/SRS&gt; &lt;SRS&gt;EPSG:5678&lt;/SRS&gt;.
     * </p>
     * 2. Deprecated: Single list-valued &lt;SRS&gt; element: a list of SRS values is represented
     * asa whitespace-separated list of SRS names contained within a single &lt;SRS&gt; element.
     ...
     * rectangle of the map data in the EPSG:4326 geographic coordinate system. The
     * LatLonBoundingBox attributes minx, miny, maxx, maxy indicate the edges of an enclosing
     * rectangle in decimal degrees. LatLonBoundingBox shall be supplied regardless of what SRS the
     * map server may support, but it may be approximate if EPSG:4326 is not supported. Its purpose
     * is to facilitate geographic searches without requiring coordinate transformations by the
     * search engine.
     * 
     * @return the bbox
     */
    ...
    /**
     * sets the LatLonBoundingBox element that is either stated explicitly or inherited from a
     * parent Layer.
     * 
     * @param latLonBoundingBox
     */
     ...
     * or inherited from a parent Layer. Each BoundingBox states the bounding rectangle of the map
     * data in a particular spatial reference system; the attribute SRS indicates which SRS applies.
     * If the data area is shaped irregularly then the BoundingBox gives the minimum enclosing
     * rectangle. The attributes minx, miny, maxx, maxy indicate the edges of the bounding box in
     * units of the specified SRS. Optional resx and resy attributes indicate the spatial resolution
     * of the data in those same units.
     * <p>
     * A Layer may have multiple BoundingBox element, but each one shall state a different SRS. A
     * Layer inherits any BoundingBox values defined by its parents. A BoundingBox inherited from
     * the parent Layer for a particular SRS is replaced by any declaration for the same SRS in the
     * child Layer. A BoundingBox in the child for a new SRS not already declared by the parent is
     * added to the list of bounding boxes for the child Layer. A single Layer element shall not
    ...
    /**
     * Dimension declarations are inherited from parent Layers. Any new Dimension declarations in
     * the child are added to the list inherited from the parent. A child shall not redefine a
     * Dimension with the same name attribute as one that was inherited.
     * 
    ...
    /**
     * Extent declarations are inherited from parent Layers. Any Extent declarations in the child
     * with the same name attribute as one inherited from the parent replaces the value declared by
     * the parent. A Layer shall not declare an Extent unless a Dimension with the same name has
     * been declared or inherited earlier in the Capabilities XML.
     * 
     * @return the extents
     */
     ...
     * data used in a Layer or collection of Layers. Attribution encloses several optional elements:
     * <OnlineResource>states the data provider's URL; &lt;Title&gt; is a human-readable string
     * naming the data provider; &lt;LogoURL&gt; is the URL of a logo image. Client applications may
     * choose to display one or more of these items. A &lt;Format&gt; element in LogoURL indicates
     * the MIME type of the logo image, and the attributes width and height state the size of the
     * image in pixels.
     * 
     ...
     * separate &lt;AuthorityURL&gt; element. AuthorityURL encloses an &lt;OnlineResource&gt;
     * element which states the URL of a document defining the meaning of the Identifier values.
     * 
     ...
     * indicates the standard to which the metadata complies. Two types are defined at present: the
     * value 'TC211' refers to [ISO 19115]; the value 'FGDC' refers to [FGDC-STD-001-1988]. The
     * MetadataURL element shall not be used to reference metadata in a non-standardized metadata
     * format; see DataURL instead. The enclosed &lt;Format&gt; element indicates the file format
     ...
     * particular layer. While the semantics are not well-defined, as long as the results of an HTTP
     * GET request against the DataURL are properly MIME-typed, Viewer Clients and Cascading Map
     * Servers can make use of this. Use 6lt;MetadataURL&gt; instead for a precisely defined
    ...
    /**
     * returns the <tt>UserStyle</tt> (SLD) representation of the style identified by the
     * submitted name.
     * 
     * @param name
     ...
     * which can be expressed as the ground distance in meters of the southwest to northeast
     * diagonal of that pixel. The two values in ScaleHint are the minimum and maximum recommended
     * values of that diagonal. It is recognized that this definition is not geodetically precise,
     * but at the same time the hope is that by including it conventions will develop that can be
     * later specified more clearly.
     * 
    ...
    /**
     * removes a Layer identified by its name from the parent Layer. A reference to the removed
     * layer will be returned. If no Layer matching the passed name can be found nothing happens and
    ...
    /**
     * removes a Layer identified by its title from the parent Layer. A reference to the removed
     * layer will be returned. If no Layer matching the passed title can be found nothing happens
    ...
    /**
     * @return the parent layer of this layer. If the method returns <tt>null</tt> the current
     *         layer is the root layer. In addition with the <tt>getLayer</tt> method this enables
     *         a program to traverse the layer tree in both directions.
     */
    ...
    /**
     * @return '0' if the layer is provided directly form the deegree WMS. other it returns the
     *         number of cascaded WMS servers the is passed through
     * 
     */
    ...
    /**
     * @return '0' if the WMS can resize map to arbitrary height. nonzero: map has a fixed height
     *         that cannot be changed by the WMS.
     * 
     */
    ...
    /**
     * @return '0' if the WMS can resize map to arbitrary width. nonzero: map has a fixed width that
    ...
    /**
     * @return false if map data represents vector features that probably do not completely fill
     *         space.
     * 
     */
    ...
    /**
     * @return true if the layer is queryable. That means it can be targeted by a GetFeatureInfo
     *         request.
     * 
     */
