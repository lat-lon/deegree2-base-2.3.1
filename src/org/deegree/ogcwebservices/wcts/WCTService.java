...
/**
 * The <code>WCTService</code> class is the interface between the actual handling of an incoming
 * request and the configuration, key method is the {@link #doService(OGCWebServiceRequest)}
 * implementation.
 * 
...
public class WCTService implements OGCWebService {
    ...
    public Object doService( OGCWebServiceRequest request )
        ...
        if ( request != null ) {
            ...
            if ( request instanceof GetResourceByID ) {
                ...
                throw new OGCWebServiceException( Messages.getMessage( "WCTS_OPERATION_NOT_SUPPORTED",
                                                                       "GetTransformation" ),
                                                  WCTSExceptionCode.OPERATIONNOTSUPPORTED );
        ...
        if ( result == null ) {
            ...
            throw new OGCWebServiceException( Messages.getMessage( "WCTS_ILLEGAL_STATE" ),
                                              WCTSExceptionCode.NOAPPLICABLECODE );
        }
     ...
     * @param request
     *            to be handled.
     * @return the response to a GetResourceByID request (i.e. a gml:Dictionary with the description
     *         of the resource in gml3 ).
     */
    ...
    private XMLFragment handleIsTransformable( IsTransformable request ) {
        ...
        if ( geometriesFitConfigured ) {
            // the requested geometries did fit the configured geometries, but does deegree support
            // them too?
     ...
     * @throws OGCWebServiceException
     *             if the inputdata could not be found, or the given crs are not configured to be
     *             supported.
     */
    public TransformResponse handleTransform( Transform request )
        ...
        transformableData.doTransform( request.extensiveLogging() );
        return new TransformResponse( request.getDataPresentation(), request.mustStore(), request.getInputData(),
                                      transformableData );
    }
