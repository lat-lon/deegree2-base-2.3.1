...
public class Transform extends WCTSRequestBase {
    ...
    /**
     * A private constructor for creation of kvp, assuming therefore that the presented data is a FeatureCollection and
     * setting dataPresentation to {@link #MULTIPART}.
     * 
     * @param version
     ...
     * @param targetCRS
     * @param inputData
     *            a string containing a location to load the input data from.
     * @param outputFormat
     * @throws OGCWebServiceException
     *             if no data could be loaded from the given location (inputData).
     */
    private Transform( String version, String requestID, boolean store, CoordinateSystem sourceCRS,
                       CoordinateSystem targetCRS, String inputData, String outputFormat )
                            throws OGCWebServiceException {
        ...
        List<FeatureCollection> dataList = new ArrayList<FeatureCollection>( 1 );
        FeatureCollection data = loadFeatureCollection( inputDataString );
        if ( data != null ) {
            dataList.add( data );
        } else {
            LOG.logError( "No data found at location: " + inputDataString );
            throw new OGCWebServiceException( Messages.getMessage( "WCTS_TRANSFORM_MISSING_DATA" ),
                                              WCTSExceptionCode.NO_INPUT_DATA );
        }
        this.transformableData = new FeatureCollectionData( sourceCRS, targetCRS, dataList );
    ...
    public Transform( String version, String requestID, boolean store, CoordinateSystem sourceCRS,
                      CoordinateSystem targetCRS, Manifest inputData, TransformableData<?> transformableData,
                      String outputFormat, int dataPresentation ) throws IllegalArgumentException {
        super( version, requestID, null );
        this.store = store;
        this.sourceCRS = sourceCRS;
        this.targetCRS = targetCRS;
    ...
    /**
     * Try to load the featureCollection from a given url.
     * 
     * @param url
     *            to load the data from.
     * @return a loaded feature collection or <code>null</code> if no file could be loaded.
     */
    private FeatureCollection loadFeatureCollection( String url ) {
        if ( url == null ) {
            return null;
        }
    ...
    public static Transform create( String requestID, Map<String, String> map )
        ...
        if ( ( ( sCRS != null && !"".equals( sCRS.trim() ) ) && ( tCRS == null || "".equals( tCRS.trim() ) ) ) || ( ( tCRS != null && !"".equals( tCRS.trim() ) ) && ( sCRS == null || "".equals( sCRS.trim() ) ) ) ) {
        ...
        return new Transform( version, requestID, store, sourceCRS, targetCRS, inputData, outputFormat );
    }
